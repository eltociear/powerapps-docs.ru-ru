...
2. На вкладке **Поля** можно выбрать следующие представления. 

 |Представление|Описание|
 |--|--|
 |**Все**| Отображаются все поля для сущности|
 |**Управляемый**| Отображаются только управляемые и стандартные поля для сущности|
...
|**Автонумерация**|Настраиваемые сочетание цифр и букв, которое создается автоматически сервером при создании записи. Дополнительные сведения: [Поля с автоматической нумерацией](autonumber-fields.md) |
...
|**Длительность**|Числовое значение, представленное как раскрывающийся список, содержащий интервалы времени. Пользователь может выбрать значение в списке или ввести целое число, представляющее количество минут. Длительность следует вводить в формате: "x мин", "x ч" или "x дней". Часы и дни также можно вводить, используя десятичные числа, например "x.x ч" или "x.x дней". Введенные значения должны быть выражены в минутах, значения меньше минуты округляются до одной минуты.|
...
|**Поиск**| Создает ссылку на одну запись для одного целевого типа записей.|
