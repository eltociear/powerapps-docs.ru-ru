...
description: 'Узнайте, как импортировать, обновить и экспортировать решение в PowerApps'
ms.custom: ''
ms.date: 11/06/2018
ms.reviewer: ''
...
# <a name="import-update-and-export-solutions"></a>Импорт, обновление и экспорт решений 
 ...
 Решения можно импортировать вручную, используя описанные ниже действия. Импортируйте только решения,полученные из надежного источника. Настройки могут включать код, передающий данные во внешние источники. Решение с именем **Решение по умолчанию** можно импортировать только в среду, из которой оно было экспортировано, но не в другую среду.  
...
## <a name="export-solutions"></a>Экспорт решений  
 Рекомендуется периодически экспортировать неуправляемые настройки, чтобы у вас была резервная копия на всякий случай. Экспортировать управляемые решения невозможно. Решения можно экспортировать из PowerApps или с помощью классического интерфейса. 
 
### <a name="export-from-powerapps"></a>Экспорт из PowerApps
  
1.  Выберите **Решения** в левой области навигации.   
  
2.  В списке выберите решение, которое нужно экспортировать, и щелкните **Экспорт**. 

3.  Выберите тип пакета **Как неуправляемое** или **Как управляемое**. После этого запустится экспорт, выполнение которого может занять несколько минут. По завершении ZIP-файл экспорта будет доступен в папке загрузки, указанной в веб-браузере.

    > [!div class="mx-imgBorder"]  
    > ![Экспорт решения](media/solution-export.png "Экспорт решения") 
...
## <a name="settings-options-for-solution-export"></a>Задание параметров для экспорта решения  
 При экспорте решения из PowerApps пропустите этот раздел. В следующей таблице описываются параметры, доступные при экспорте решения из классического интерфейса:  
...
|Код форматирования даты|Сведения о формате отображения даты в Dynamics 365 for Customer Engagement|  
