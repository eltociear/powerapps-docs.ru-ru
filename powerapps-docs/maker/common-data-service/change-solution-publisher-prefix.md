...
ms.locfileid: "2861706"
---
# <a name="change-the-solution-publisher-prefix"></a>Изменение префикса издателя решения

Каждая настройка является частью решения. Каждое решение имеет издателя. По умолчанию решение, с которым вы будете работать в Power Apps, будет **Решение по умолчанию служб Common Data Service**, связанное с параметром **Издатель Common Data Service по умолчанию**.

Префикс настройки по умолчанию будет случайно назначен для этого издателя, например это может быть `cr8a3`. Это означает, что имя каждого нового элемента метаданных, созданных в организации, будет иметь этот префикс рядом с именами, используемыми для уникального определения элементов. При создании новой сущности с именем **Animal** (Животное) уникальное имя, используемое Common Data Service, будет `cr8a3_animal`. То же самое верно для всех новых полей (атрибутов), отношений или параметров набора параметров.
...
## <a name="change-the-solution-publisher-prefix-for-the-common-data-service-default-publisher"></a>Изменение префикса издателя решения для издателя Common Data Service по умолчанию  

 1. На портале Power Apps выберите **Управляемые моделью** в левом нижнем углу.
 ...
 4. Перейдите по ссылке **Издатель**, чтобы открыть форму **Издатель Common Data Service по умолчанию**.
