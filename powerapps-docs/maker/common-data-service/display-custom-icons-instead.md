---
title: Отображение настраиваемых значков вместе со значениями в представлениях списка с помощью Power Apps | Документация Майкрософт
...
В этом примере отображаются настраиваемые значки в представлении сущности возможной сделки, которое доступно в некоторых приложениях, таких как Dynamics 365 Sales. Вы можете отображать настраиваемые значки в представлениях с другими стандартными сущностями, такими как учетная запись или сущность контакта, а также с настраиваемыми сущностями. 
...
### <a name="sample-javascript-function"></a>Пример функции JavaScript  
 Функция JavaScript для отображения настраиваемых значков и подсказок ожидает следующие два аргумента: всю строку объекта, указанного в layoutxml, и идентификатор локали вызывающего пользователя (LCID). Параметр LCID позволяет указать текст подсказок на нескольких языках. Дополнительные сведения о языках, поддерживаемых средой, см. в разделах [Включение языков](/dynamics365/customer-engagement/admin/enable-languages) и [Установка или обновление языковых пакетов](/dynamics365/customer-engagement/on-premises/install-or-upgrade-language-packs). Список значений кодов языков (LCID), которые можно применять в коде, см. в статье [Коды языков, назначенные Microsoft](https://go.microsoft.com/fwlink/?linkid=829588).
...
> [!IMPORTANT]
> Для этого примера требуется сущность возможной сделки, которая доступная с приложениями Dynamics 365 Sales.
...
## <a name="custom-icon-view-display-behavior"></a>Поведение отображения представления настраиваемых значков
### <a name="primary-fields"></a>Основные поля 
В представлении списка сетки настраиваемые значки, примененные к основному полю сущности, заменяют созданный системой значок по умолчанию. 

> [!div class="mx-imgBorder"] 
> ![](media/mobile-primary-field-custom-icon-display.png "Primary field replaces default icon in the custom icon view")

### <a name="other-fields"></a>Другие поля 
В представлении списка сетки настраиваемые значки, примененные к полю, которое не является основным полем сущности, отображаются в качестве вспомогательного значка в дополнение к значку, созданному системой по умолчанию. 

> [!div class="mx-imgBorder"] 
> ![](media/card-form-not-primary-field.png "Not an entity primary field custom icon view")

### <a name="card-forms"></a>Формы карточки
...
[Общие сведения о представлениях управляемых моделью приложений](../model-driven-apps/create-edit-views.md)
