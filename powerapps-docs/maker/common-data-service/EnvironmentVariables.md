...
## <a name="how-do-they-work"></a>Как они работают?
...
### <a name="create-an-environment-variable-in-power-apps"></a>Создание переменной среды в Power Apps
1. Войдите в Power Apps, затем в левых областях выберите **Решения**. 
2. На панели команд выберите **Создать**, затем выберите **Переменная среды**. 
3. В левой области заполните следующие поля, затем выберите **Сохранить**:  
   ...
   - **Тип данных**. Выберите из поля **Десятичное число**, **Текст**, **JSON** или **Два параметра**. 
   - **Значение по умолчанию**. Это поле является частью сущности определения переменной среды и не является обязательным. Задайте значение по умолчанию для производственных сред или если значения не требуется изменять для разных сред. 
   - **Текущее значение**. Также известно как значение переопределения. Это поле является необязательным и является частью сущности значения переменной среды. Задайте значение, если нужно переопределить значение по умолчанию в текущей среде. Удалите значение из своего решения, если не хотите использовать его в следующей среде. Значения также разделены в отдельный файл JSON в файле solution.zip, который экспортируется. 

      Разделение значения по умолчанию и текущего значения позволяет обслуживать определение и значение по умолчанию отдельно от текущего значения. Оно также позволяет нам расширить функциональность в будущем, чтобы поддерживать несколько значений, ограниченных определенным контекстом времени выполнения.
...
- Приложения холста и потоки могут использовать переменные среды как данные записи сущности. <!-- In the future we plan to build additional actions into canvas app and flow designers. This will simplify authoring and provide better visibility into environment variables being used by a specific app or flow. -->
- Интеграция Azure Key Vault для управления секретами. В настоящее время переменные среды не должны использоваться для хранения защищенных данных, таких как пароли и ключи.
...
[Блог Power Apps: переменные среды доступны в предварительной версии!](https://powerapps.microsoft.com/blog/environment-variables-available-in-preview/)
