...
description: Устранение неполадок путем использования Power Query для создания настраиваемой сущности в Common Data Service.
author: mllopis
manager: kfile
...
Эта ошибка появляется, если Power Query не удается получить доступ к данным организации в PowerApps или Common Data Service. Эта ситуация возникает в двух наборах ситуаций:
...
В PowerApps вы можете получить доступ к редактору запросов, выполнив следующие действия.
1. Перейдите на панель **Данные**, разверните ее, затем выберите **Сущности**.
2. Выберите многоточие (...), затем выберите **Изменить запросы**. 
3. На ленте выберите кнопку **Параметры**, затем выберите кнопку **Экспорт диагностики**.
