...
В Power Apps большинство приложений Canvas используют внешние данные, хранящиеся в облачных службах, называемых **источниками данных**. Типичный пример — это таблица в файле Excel, который хранится в OneDrive для бизнеса. Приложения получают доступ к источникам данных с помощью **подключений**.
...
При запросе приложений Power Apps для создания приложения из данных используются эти элементы управления. Приложение использует внутреннюю таблицу для хранения и обработки данных, поступающих из источника данных.
...
Таблицы, которые являются внутренними для приложения Power Apps, являются фиксированными значениями, как число или строка — это значение. Внутренние таблицы не хранятся, они просто находятся в памяти приложения. Нельзя напрямую изменять структуру и данные таблицы. Вместо этого вы можете создать новую таблицу по формуле, с помощью которой можно создать измененную копию исходной таблицы.

Внешние таблицы хранятся в источнике данных для последующего извлечения и совместного использования.  Power Apps предоставляет "подключения" для чтения и записи сохраненных данных.  По подключению можно обращаться к нескольким таблицам данных.  Вы можете выбрать таблицы, требуемые для использования в приложении, каждая из которых станет отдельным *источником данных*.  
...
Источники табличных данных можно использовать так же, как и внутренние таблицы Power Apps.  Так же как и внутренняя таблица, каждый источник данных имеет [записи](working-with-tables.md#records), [столбцы](working-with-tables.md#columns) и свойства, которые можно использовать в формулах. Кроме того:
...
Power Apps нельзя использовать для создания подключенного источника данных или изменения его структуры; Источник данных уже должен существовать в какой-либо службе. Например, чтобы создать таблицу в книге Excel, хранимой в OneDrive, сначала требуется создать книгу с помощью Excel Online в OneDrive. Затем следует подключить приложение к ней.  
...
Power Apps предлагает два средства для проверки:
...
Если ошибка возникает при создании записи с помощью функции **[Patch](functions/function-patch.md)** или **[Collect](functions/function-clear-collect-clearcollect.md)** , запись, с которой можно было бы связать ошибку, отсутствует.  В этом случае функцияPatch **[ вернет значение ](functions/function-patch.md)blank**, которое может использоваться в качестве аргумента записи в функции **[Errors](functions/function-errors.md)** .  Ошибки при создании удаляются со следующей операцией.
