...
description: В Power Apps создание привлекательных макетов форм в приложениях Canvas с помощью строк и столбцов.
author: gregli-msft
manager: kvivek
...
Если вы не знакомы с приложениями Power Apps (или создали только автоматически созданные приложения), вам потребуется [создать приложение с нуля](get-started-create-from-blank.md) , прежде чем приступить к работе с этим разделом. Создав приложение с нуля, вы ознакомитесь с такими важными понятиями, как добавление источников данных и элементов управления, которые упоминаются, но подробно не рассматриваются в этой статье.

Этот раздел написан так, как если бы у вас был источник данных с именем « **заказ на продажу** » и он содержит поля на предыдущем рисунке. Если у вас есть лицензия Power Apps с планом 2 или [пробная лицензия](../signup-for-powerapps.md) и системный администратор или права системного администратора, можно [создать сущность](../common-data-service/data-platform-create-entity.md) в Common Data Service и добавить похожие поля. 

## <a name="add-a-gallery"></a>Добавление коллекции

1. Создайте приложение планшета с нуля и добавьте источник данных.
...
2. Добавьте вертикальный элемент управления **Коллекция** и укажите для его свойства **Items** значение **Sales order**.
...
Иногда может потребоваться более точное управление, чем могут обеспечить стандартные 12 столбцов. В этих случаях можно отключить функцию **Прикрепление к столбцам** и разместить карточки вручную. Форма будет привязана к 12 столбцам, но можно также удерживать нажатыми клавиши ALT или Ctrl + Shift после начала изменения размера или перемещения, чтобы переопределить точки привязки.  Дополнительные сведения см. в разделе [альтернативное поведение сочетания клавиш](keyboard-shortcuts.md#alternate-behavior). 

В нашем примере третья строка адреса состоит из четырех компонентов одинаковой ширины. Возможно, такой макет будет далек от идеального, так как название города часто бывает длиннее сокращения для названия области, республики, края или округа, и поле для ввода страны или региона уменьшается из-за длины его метки.  Чтобы оптимизировать это пространство, отключите **привязку к столбцам** на правой панели, а затем удерживайте нажатыми клавиши ALT или Ctrl + Shift после начала изменения размера и положения этих карточек. Для
...
| Функция | Прикрепление к столбцам включено | Прикрепление к столбцам отключено |
| --- | --- | --- |
| Прикрепление при изменении размера |Для выбранного числа столбцов:<br>1, 2, 3, 4, 6 или 12. |Для 12 столбцов |
| Возможность переопределить прикрепление при изменении размера |Нет |Да, с клавишами Alt или Ctrl + Shift после начала изменения размера |
