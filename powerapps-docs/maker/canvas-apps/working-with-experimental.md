...
1. **Экспериментальные**:  Эта функция находится на стадии. На нее пока не следует рассчитывать, так как она может претерпеть значительные изменения.
1. **Предварительная версия**:  Эта функция почти готово и достижения стабильности в работе. Можно начать процесс ее внедрения в существующие приложения.
1. **Поставляется**:  Эта функция выполняется. Эта функция включена во всех приложениях и ее невозможно отключить.
...
| Этап | Когда его использовать? | Ее можно использовать с полной уверенностью? | Она включена по умолчанию для новых приложений? | 
|----|----|----|-----|------|
...
В заключительный этап **Shipped**, мы может удалить switch предварительной версии в приложениях, для которых уже включена, эффективно Включение функции без возможности восстановления. Это изменение будет применено для большинства приложений, так как эта функция будет содержаться на по умолчанию до этого момента. Для приложений, где функция отключена Предварительная версия по-прежнему будет доступен коммутатор можно включить, тестирование с помощью функции и отключить внутри одного сеанса PowerApps Studio. Тем не менее при сохранении приложения, когда параметр включен, он не будут доступны при приложение перезапускается, поэтому невозможно включить эту функцию снова. На этом этапе вы можете [восстановить приложение до предыдущей версии](restore-an-app.md) для возвращения приложения в состояние, прежде чем был включен компонент.

## <a name="documentation"></a>Документация по
