...
1. **Экспериментальный**:  Эта функция находится в процессе работы. На нее пока не следует рассчитывать, так как она может претерпеть значительные изменения.
1. **Предварительный просмотр**:  Эта функция почти создана и стабильной. Можно начать процесс ее внедрения в существующие приложения.
1. **Поставка**:  Эта функция реализована. Эта функция включена во всех приложениях и ее невозможно отключить.
...
В последнем переходе к **поставке**мы можем удалить параметр предварительного просмотра в приложениях, для которых эта функция уже включена, эффективно переключив эту функцию на постоянной основе. Это изменение будет применено к большинству приложений, поскольку функция будет включена по умолчанию до этой точки. Для приложений, в которых эта функция отключена, параметр Предварительный просмотр по-прежнему будет доступен для включения, тестирования с помощью функции и отключения в рамках одного сеанса PowerApps Studio. Однако если вы сохраняете приложение при включенном параметре, оно не будет доступно при повторной загрузке приложения, поэтому вы не сможете снова отключить эту функцию. На этом этапе можно [восстановить приложение до предыдущей версии](restore-an-app.md) , чтобы вернуть приложение в состояние до включения компонента.
