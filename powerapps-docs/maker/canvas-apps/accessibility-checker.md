...
1. В правом верхнем углу окна Power Apps Studio выберите значок средства проверки приложений.
...
| Название проблемы                            | Серьезности | Описание проблемы  | Способ устранения | Причина устранения|
| ------------------------------         |:---------| -----| ------|------ |
...
| **HTML не будет доступен**           | Предупреждение  | Если элемент управления, отличный от текста HTML, содержит HTML. В этом случае Power Apps не поддерживает доступность пользовательских HTML-элементов. | Используйте другой метод вместо HTML или удалите HTML из этого элемента. | Ваше приложение будет работать неправильно или не будет поддерживать специальные возможности, если вы добавите интерактивные элементы HTML. |
...
| **Проверить порядок элементов экрана**| Совет      | Если свойство **TabIndex** больше 0. Создатели приложений могут устанавливать пользовательские заказы на табуляцию, присвоив свойству **TabIndex** значение больше 0, но это крайне нежелательно, так как трудно получить право, поддерживать и разрушить средства чтения с экрана. | При возможности задайте для свойств **TabIndex** значение 0 или-1.  Вместо использования **TabIndex**используйте улучшенный элемент управления " **Группа** ", чтобы изменить порядок навигации по умолчанию.  Если необходимо использовать значения **TabIndex** больше 0, убедитесь, что элементы экрана соответствуют порядку, в котором их следует прокручивать. | Порядок навигации должен отражать порядок, в котором элементы управления отображаются на экране, что является значением по умолчанию.  Если выполняется ручная корректировка, то трудно поддерживать правильный порядок, особенно при наличии адресной строки браузера и других элементов управления за пределами приложения.  Это может привести к очень трудному использованию средства чтения с экрана.  При чтении средством чтения с экрана элементы управления должны быть представлены в том же порядке, в котором они отображаются на экране, а не в том порядке, который менее интуитивен.  |
