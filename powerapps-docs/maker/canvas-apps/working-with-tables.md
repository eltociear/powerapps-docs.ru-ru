...
Чтобы сослаться в формуле на саму запись вне контекста таблицы, воспользуйтесь фигурными скобками. Например, эта запись **{Name: "Клубника"; Цена: 7;99}** не связан с таблицей. Обратите внимание, что имена полей (**Name** и **Price** в этом примере) не заключаются в двойные кавычки.
...
Рассмотрим функцию **нижнего уровня** . Если переменная **Welcome** содержит текстовую строку **"Hello, World"** , формула **ниже (Welcome)** возвращает **"Hello, World"** .  Эта функция не позволяет изменить значение в этой переменной. Функция **Lower** является чистой функцией, которая обрабатывает только входные данные и выдает результат. Все это, Он не имеет побочных эффектов. Все функции в Excel и большинство функций в PowerApps — чистые функции, позволяющие автоматически повторно вычислять книгу или приложение.
...
Чтобы создать этот пример таблицы в приложении, вставьте кнопку, задайте для этой формулы свойство onSelect, а затем нажмите кнопку (щелкните ее, удерживая нажатой клавишу Alt в PowerApps Studio):
...
Кроме того, определите переменную контекста с именем **value** с помощью следующей формулы: **Упдатеконтекст ({Value: "!"})**
...
Например, `[ 1; 2; 3; 4 ]` `Table( { Value: 1 }; { Value: 2 }; { Value: 3 }; { Value: 4 } )` эквивалентна и возвращает следующую таблицу:
