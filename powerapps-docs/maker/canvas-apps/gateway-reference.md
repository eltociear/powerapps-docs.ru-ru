...
Шлюз создает исходящее подключение к служебной шине Azure. Осуществляет связь через исходящие порты: TCP-порт 443 (по умолчанию), 5671, 5672 и 9350 – 9354. Шлюз не нуждается во входящих портах.
...
**Ответ:** На момент написания этой статьи:
...
**Вопрос.** Нужно устанавливать шлюз для источников данных в облаке, например SQL Azure?  
**Ответ:** Нет. Шлюз подключается только к локальным источникам данных.

**Вопрос.** То, что называется фактической службы Windows  
**Ответ:** В службах, шлюз называется **Power BI Enterprise Gateway Service**.

**Вопрос.** Существуют входящие подключения к шлюзу из облака?  
**Ответ:** Нет. Шлюз использует исходящие подключения к служебной шине Azure.

**Вопрос.** Что делать, если мной заблокированы исходящие подключения? Что нужно открыть?  
**Ответ:** Ознакомьтесь со списком портов и узлов, которые шлюз использует выше.

**Вопрос.** Имеет ли шлюз быть установлен на том же компьютере в качестве источника данных?  
...
**Вопрос.** Что такое задержка при выполнении запросов к источнику данных из шлюза? Какую архитектуру лучше использовать?  
**Ответ:**  Чтобы уменьшить задержки в сети, установите шлюз как можно ближе к источнику данных максимально. Если есть возможность установить шлюз на компьютере-источнике данных, это минимизирует задержки. Также можно рассмотреть вариант использования центра данных. Например если служба использует центр данных "Западная часть США" и есть SQL Server, размещенный на виртуальной машине Azure, следует поместить эту виртуальную машину Azure также в центр данных "Западная часть США". Это позволит свести к минимуму задержки и избежать расходов на исходящие данные на виртуальной машине Azure.

**Вопрос.** Существуют ли все требования к пропускной способности сети?  
**Ответ:** Рекомендуется использовать широкополосное сетевое подключение. Каждое окружение имеет свои особенности, на результат также влияет объем отправляемых данных. С помощью ExpressRoute можно обеспечить гарантированную пропускную способность от локальной системы до центров данных Azure.
...
**Вопрос.** Можно ли запускать службу Windows шлюза с учетной записью Azure Active Directory?  
**Ответ:** Нет. Служба Windows должна использовать допустимую учетную запись Windows. По умолчанию это *NT SERVICE\PBIEgwService*.

**Вопрос.** Как результаты отправляются обратно в облаке?  
**Ответ:** Это выполняется посредством служебной шины Azure. Дополнительные сведения см. в разделе [Принцип работы](gateway-reference.md#how-the-gateway-works).

**Вопрос.** Где хранятся мои учетные данные?  
**Ответ:** Учетные данные, введенные для источника данных хранятся в облачной службе шлюза в зашифрованном виде. Учетные данные расшифровываются на локальном шлюзе.

**Вопрос.** Можно ли поместить шлюз в сети периметра (также называется DMZ, демилитаризованной зоной и промежуточной подсетью)?  
...
**Вопрос.** Существуют ли все планы по реализации сценариев высокого уровня доступности с помощью шлюза?  
**Ответ:** Высокий уровень доступности включен, объединяя 2 или несколько шлюзов в том же кластере.  Высокий уровень доступности для кластеров требует обновления от ноября 2017 г. или более поздней версии для локального шлюза данных.  Дополнительные сведения см. в [объявлении в записи блога](https://powerapps.microsoft.com/en-us/blog/gateway-high-availability-for-powerapps-and-flow).

**Вопрос.** Какие варианты доступны для аварийного восстановления?  
**Ответ:** Можно использовать ключ восстановления для восстановления или переноса шлюза. При установке шлюза укажите ключ восстановления.

**Вопрос.** В чем заключается преимущество ключа восстановления?  
**Ответ:** Он позволяет перенести или восстановить параметры шлюза после аварии.

#### <a name="troubleshooting"></a>Устранение неполадок
**Вопрос.** Где находятся журналы шлюза?  
**Ответ:** См. в разделе [средства](gateway-reference.md#tools) далее в этом разделе.

**Вопрос.** Как узнать, что запросы отправляются к локальному источнику данных?  
**Ответ:** Вы можете включить трассировку запросов, которая будет включать отправленные запросы. Не забудьте выключить трассировку после устранения неполадок. Включенная трассировка запросов приводит к росту размера журналов.
