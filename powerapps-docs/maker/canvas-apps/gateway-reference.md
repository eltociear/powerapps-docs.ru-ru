...
Шлюз создает исходящее подключение к служебной шине Azure. Он взаимодействует с исходящими портами: TCP 443 (по умолчанию), 5671, 5672, 9350 – 9354. Шлюз не нуждается во входящих портах.
...
**Вопрос** Нужен ли шлюз для источников данных в облаке, например SQL Azure?  
**Клик** Нет. Шлюз подключается только к локальным источникам данных.

**Вопрос** Что называется фактической службой Windows?  
**Клик** В службах шлюз называется **локальной службой шлюза данных**.

**Вопрос** Существуют ли входящие подключения к шлюзу из облака?  
**Клик** Нет. Шлюз использует исходящие подключения к служебной шине Azure.

**Вопрос** Что делать, если я блокирую исходящие подключения? Что нужно открыть?  
**Клик** См. список портов и узлов, используемых шлюзом выше.

**Вопрос** Должен ли шлюз быть установлен на том же компьютере, что и источник данных?  
**Клик** Нет. Шлюз будет подключаться к источнику данных, используя указанные сведения для подключения. В этом смысле шлюз можно рассматривать как клиентское приложение. Он просто должен иметь возможность подключаться к указанному имени сервера.

**Вопрос** Какова задержка при выполнении запросов к источнику данных из шлюза? Какую архитектуру лучше использовать?  
**Клик**  Чтобы снизить задержку в сети, установите шлюз как можно ближе к источнику данных. Если есть возможность установить шлюз на компьютере-источнике данных, это минимизирует задержки. Также можно рассмотреть вариант использования центра данных. Например если служба использует центр данных "Западная часть США" и есть SQL Server, размещенный на виртуальной машине Azure, следует поместить эту виртуальную машину Azure также в центр данных "Западная часть США". Это позволит свести к минимуму задержки и избежать расходов на исходящие данные на виртуальной машине Azure.

**Вопрос** Существуют ли требования к пропускной способности сети?  
**Клик** Рекомендуется иметь хорошую пропускную способность для сетевого подключения. Каждое окружение имеет свои особенности, на результат также влияет объем отправляемых данных. С помощью ExpressRoute можно обеспечить гарантированную пропускную способность от локальной системы до центров данных Azure.
...
**Вопрос** Может ли служба Windows шлюза запускаться с учетной записью Azure Active Directory?  
**Клик** Нет. Служба Windows должна использовать допустимую учетную запись Windows. По умолчанию это *NT SERVICE\PBIEgwService*.

**Вопрос** Как результаты отправляются обратно в облако?  
**Клик** Это делается с помощью служебной шины Azure. Дополнительные сведения см. в разделе [Принцип работы](gateway-reference.md#how-the-gateway-works).

**Вопрос** Где хранятся мои учетные данные?  
**Клик** Учетные данные, введенные для источника данных, хранятся в облачной службе шлюза в зашифрованном виде. Учетные данные расшифровываются на локальном шлюзе.

**Вопрос** Можно ли разместить шлюз в сети периметра (также называемой демилитаризованной зоной, промежуточной подсетью)?  
**Клик** Шлюзу требуется подключение к источнику данных. Если источник данных не находится в сети периметра, шлюз может не иметь возможности подключиться к нему. Например, компьютер, на котором работает SQL Server, может не находиться в сети периметра. В этом случае подключиться к нему из сети периметра невозможно. Если шлюз находится в сети периметра, он не сможет обращаться к компьютеру, на котором выполняется SQL Server.

#### <a name="high-availabilitydisaster-recovery"></a>Высокая доступность и аварийное восстановление
**Вопрос** Существуют ли планы для включения сценариев высокой доступности с помощью шлюза?  
**Клик** Высокий уровень доступности включается путем объединения двух или более шлюзов в один кластер.  Высокий уровень доступности для кластеров требует обновления от ноября 2017 г. или более поздней версии для локального шлюза данных.  Дополнительные сведения см. в [объявлении в записи блога](https://powerapps.microsoft.com/en-us/blog/gateway-high-availability-for-powerapps-and-flow).

**Вопрос** Какие варианты доступны для аварийного восстановления?  
**Клик** Ключ восстановления можно использовать для восстановления или перемещения шлюза. При установке шлюза укажите ключ восстановления.

**Вопрос** Каково преимущество ключа восстановления?  
**Клик** Он обеспечивает возможность миграции или восстановления параметров шлюза после аварии.

#### <a name="troubleshooting"></a>Устранение неполадок
**Вопрос** Где находятся журналы шлюза?  
**Клик** См. раздел [средства](gateway-reference.md#tools) далее в этом разделе.

**Вопрос** Как можно увидеть, какие запросы отправляются в локальный источник данных?  
**Клик** Можно включить трассировку запросов, которая будет включать отправляемые запросы. Не забудьте выключить трассировку после устранения неполадок. Включенная трассировка запросов приводит к росту размера журналов.
