...
Делегирование поддерживается только определенных источников табличных данных. Если источник данных поддерживает делегирование, его [документации по соединителям](https://docs.microsoft.com/connectors/) описаны, поддерживающие. Например эти табличных источников данных являются самыми популярными, и они поддерживают делегирование:

- [Common Data Service](https://docs.microsoft.com/connectors/commondataservice/) 
- [SharePoint](https://docs.microsoft.com/connectors/sharepointonline/) 
- [SQL Server](https://docs.microsoft.com/connectors/sql/) 

Импортировать книги Excel (с помощью **добавление статических данных в приложение** источника данных), коллекции и таблицы, хранимые в переменных контекста, которые не требуют делегирования. Все эти данные уже находятся в памяти и поддерживают все функциональные возможности языка PowerApps.
...
Также можно использовать части формул, которые возвращают константные значения для всех записей. Например **слева (Language(), 2)**, **даты (2019 г., 3, 31)**, и **Today()** не зависят от всех столбцов записи и, таким образом, возвращают одинаковое значение для всех записей. Эти значения могут отправляться в источник данных как константа и не должны блокировать делегирования. 
...
* Строковые функции: **[Нижний](functions/function-lower-upper-proper.md)**,  **[верхний](functions/function-lower-upper-proper.md)**,  **[слева](functions/function-left-mid-right.md)**, **[середины](functions/function-left-mid-right.md)**,  **[Len](functions/function-left-mid-right.md)**,...
* Сигналы: **[Расположение](functions/signals.md)**,  **[ускорение](functions/signals.md)**,  **[Compass](functions/signals.md)**,...
* Меняющиеся значения: **[Функция RAND](functions/function-rand.md)**,...
...
### <a name="table-shaping-functions"></a>Формирование функции таблицы

**[AddColumns](functions/function-table-shaping.md)**,  **[DropColumns](functions/function-table-shaping.md)**,  **[RenameColumns](functions/function-table-shaping.md)**, и **[ShowColumns](functions/function-table-shaping.md)** частично поддерживают делегирование.  Формулы в их аргументов можно делегировать.  Тем не менее выходные данные этих функций, распространяются число записей не делегирования.

Как в этом примере, разработчики часто используют **AddColumns** и **подстановки** для объединения данных из одной таблицы в другую, известен как соединение в контексте баз данных:
...
Несмотря на то что **продуктов** и **поставщики** может быть Делегируемые источники данных и **подстановки** является делегируемой, функцию, выходные данные **AddColumns**функция не поддерживает делегирование. Результат выполнения всей формулы ограничены первой части **продуктов** источника данных. Поскольку функция **LookUp** и ее источник данных являются делегируемыми, совпадение со значением **Suppliers** может быть найдено в любом месте источника данных (каким бы большим он ни был). 

При использовании **AddColumns** таким образом, **подстановки** необходимо выполнить отдельные вызовы к источнику данных для каждого из этих первых записей в **продуктов**, которая вызывает огромное количество сети chatter. Если **поставщики** достаточно мала и не меняется часто, можно вызвать **собирать** работать в [ **OnStart** ](functions/signals.md) для кэширования данных источник в приложении при его запуске. Кроме того может измените приложение так, чтобы вы на включение внесенных изменений в связанных записей только в том случае, когда пользователь запрашивает для них.  
...
Например, вы автоматически создаете приложение с тремя экранами на основе таблицы SQL Server с именем **[dbo].[Fruit]**. Сведения о том, как создавать приложение, можно применить подобные принципы в [раздел, посвященный Common Data Service](data-platform-create-app.md) для SQL Server.
