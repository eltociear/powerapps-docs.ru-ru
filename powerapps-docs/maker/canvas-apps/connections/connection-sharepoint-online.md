...
description: См. в разделе доступные функции, ответы и примеры для SharePoint.
author: NickWaggoner
manager: kvivek
...
В зависимости от того, где хранятся данные можно выполнить одно или оба из следующих подходов:
...
1. В разделе **выберите список**, установите флажок для **документов** или один или несколько списков, которые вы хотите использовать, а затем выберите **Connect**:
    ...
    По умолчанию отображаются не все типы списков. PowerApps поддерживает пользовательские списки не на основе шаблона. Если имя списка, который вы хотите использовать не отображается, прокрутите вниз и введите имя списка в поле с текстом **введите пользовательское имя**.
...
- Для **Выбор** или **подстановки** столбец, укажите **ThisItem.** _ColumnName_**. Значение** для отображения данных в этом столбце.
...
- Для **пользователя или группы,** столбец, укажите **ThisItem.** _ColumnName_**. DisplayName** Показать отображаемое имя пользователя или группы.
    ...
    Вы также можете отобразить другие сведения о пользователях, например адрес электронной почты или должность. Чтобы отобразить полный список параметров, укажите **ThisItem.** _ColumnName_**.** (включая точку в конце).
...
- Для **управляемые метаданные** столбец, укажите **ThisItem.** _ColumnName_**. Метка** для отображения данных в этом столбце.
