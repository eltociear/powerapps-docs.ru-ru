...
1. Настройка формы с различными способами, например те, которые в этих разделах описывается:
...
**ОТВЕТ** Откройте форму, выберите **Копировать ссылку**и затем отправить ссылку на любой пользователь, которого вы хотите использовать форму.
...
1. Вернитесь на страницу **Версии** в другой вкладке браузера, найдите версию, которую требуется восстановить, и нажмите кнопку **Восстановить**.
    ...
    > [!NOTE]
    > Если появится сообщение о том, что восстановление не удалось, так как форма заблокирована другим пользователем, подождите, пока пользователь разблокирует форму и повторите попытку.
...
**ОТВЕТ** Вы не должны совместно использовать формы — форма наследует разрешения от списка SharePoint. Завершив настройку, просто [опубликуйте форму в SharePoint](customize-list-form.md#save-and-publish-the-form), чтобы ее могли использовать другие пользователи.
...
1. Из списка приложений, поиск приложений по **формы SharePoint** тип приложения — это настраиваемые формы.
