...
1. **Проблемы при входе на определенные мобильные устройства Android при использовании средства проверки подлинности** (21 августа, 2019)

    В некоторых устройствах и сценариях при использовании средства проверки подлинности могут возникнуть сбои при входе. Это связано с тем, что изготовители оборудования ограничивают эту функцию. Дополнительные сведения об ошибке и возможные способы их устранения см. [здесь](https://github.com/AzureAD/azure-activedirectory-library-for-android/wiki/ADALError:-BROKER_AUTHENTICATOR_NOT_RESPONDING).    

1. **Проблемы с камерой на мобильных устройствах Android** (1 января 2019 г.)

    Если управление камерой перестает работать на устройстве Android, повторно опубликуйте приложение и снова откройте его на устройстве. Элемент управления "Камера" был обновлен в ответ на изменение в операционной системе Android, и ваше приложение будет использовать преимущества обновления при повторной публикации.

1. **Прокрутка в коллекциях гибких высот** (Ноя. 27, 2018)

    Если при прокрутке с помощью пальца вы приступите к ограничению, поднимите его и начните прокручивать.

1. **Рисование с помощью мыши или сенсорного ввода не является гладким в Power Apps для Windows** (Sep. 24, 2018)

    Элемент управления "перо" имеет только частичную поддержку рисования с помощью мыши или сенсорного ввода в приложении Windows. Штрихи могут быть нерегулярными. Для плавного рисования используйте перо или запустите приложение в браузере.

1. **Несколько элементов управления мультимедиа в Power Apps Mobile** (Авг. 2, 2018)

    Power Apps Mobile работает на различных типах устройств, а некоторые из них имеют ограничения, характерные для этой платформы:
...
1. **Повторная публикация приложений** (2 августа 2018 г.)
    ...
    Если вы еще не обновили приложение в течение нескольких месяцев, повторно опубликуйте его для синхронизации с последней версией Power Apps, которая включает улучшения производительности и другие исправления.

1. <a name="out-of-memory"></a>**Нехватка памяти в браузере** (23 июля 2018 г.)

    Если при использовании Power Apps исчерпана память, попробуйте загрузить 64-разрядную версию Chrome, Microsoft ребр или Internet Explorer.
...
1. **Задание пользовательского изображения в качестве значка приложения** (11 апреля 2018 г.)
    ...
    В Power Apps Studio для Windows версии 3,18043 нельзя передать пользовательский образ для использования в качестве значка приложения. Чтобы обойти эту ошибку, используйте [Power Apps Studio для Web](https://make.powerapps.com?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc) , чтобы отправить пользовательский образ. Кроме того, можно использовать один из значков, включенных в Power Apps Studio для Windows, и настроить цвет фона.
...
1. **Элементы управления камерой в Power Apps Studio для Windows**
    ...
    Power Apps Studio для Windows может аварийно завершить работу, если добавить элемент управления камеры или открыть приложение, использующее элемент управления "Камера". Чтобы избежать этой проблемы, используйте [Power Apps Studio для Web](create-app-browser.md) при добавлении или использовании элемента управления камерой.
...
1. **Обновление Power Apps на Windows 8.1**

    Если вы устанавливаете Power Apps на компьютере под Windows 8 или Windows 8.1, храните приложение Магазина Windows в открытом и активном состоянии, используйте чудо-элемент "Параметры", чтобы проверить наличие обновлений, а затем установите их.

1. **Настраиваемые соединители и служба Common Data Service**

    Если приложение, созданное с помощью Power Apps, создает 2.0.540 или более раннюю версию, полагается на базу данных в Common Data Service и по крайней мере один настраиваемый соединитель в другой среде, необходимо развернуть соединитель в той же среде, что и база данных, и обновить приложение для использования новый соединитель. В противном случае в диалоговом окне появится сообщение о том, что API-интерфейс не найден. Подробные сведения см. в [обзоре сред](../../administrator/environments-overview.md).

1. **Запуск приложений в Windows 8.1**

    Если вы устанавливаете [это обновление для Windows 8.1](https://technet.microsoft.com/library/security/ms16-118), вы не сможете запускать приложения, открытые в Power Apps Studio в этой операционной системе. Тем не менее вы по-прежнему можете запускать приложения, открытые в [powerapps.com](https://make.powerapps.com?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc) или с помощью мобильных приложений.

1. **Имена столбцов с пробелами**

    Если вы используете список SharePoint или таблицу Excel, в которой имя столбца содержит пробел, Power Apps заменит его на **"\_x0020\_"** . Например, **"имя столбца"** в SharePoint или Excel будет отображаться как **"Column_x0020_Name"** в Power Apps, если оно отображается в макете данных или используется в формуле.
...
1. **Создание и изменение приложений в браузере**
    ...
    В Power Apps Studio для Windows можно выполнять многие и не все те же вещи, что и в Power Apps Studio. Дополнительные сведения см. в статье о [создании приложений в браузере](create-app-browser.md).
...
1. **Приложения, подключенные к локальному сайту SharePoint**
    ...
    Если предоставить общий доступ к приложению, зависящему от подключений, которые не становятся доступными автоматически (например, на локальном сайте SharePoint), то, когда пользователи откроют это приложение в браузере и нажмут кнопку **Войти**, появится диалоговое окно без текста. Чтобы закрыть диалоговое окно, щелкните значок закрыть (X) в правом верхнем углу или коснитесь его. Это диалоговое окно не отображается при открытии приложения в Power Apps Studio или Power Apps Mobile. Дополнительные сведения об общем доступе к подключениям см. в статье о [предоставлении доступа к ресурсам приложения](share-app-resources.md).

1. **Когда Power Apps создает приложение на основе данных, поле, используемое для сортировки и поиска, не настраивается автоматически**.
...
1. **В приложениях, созданных на основе данных, есть доступ только к первым 500 записям источника данных**.
     ...
     Как правило, Power Apps работает с любыми источниками данных размера, делегируя операции источнику данных. Для операций, которые не могут быть делегированы, Power Apps выдаст предупреждение во время создания и будет работать только с первыми 500 записями источника данных.  Дополнительные сведения о делегировании см. в статье о [функции фильтра](functions/function-filter-lookup.md).
...
1. **Совместное редактирование не поддерживается. По одному автору за раз**.
