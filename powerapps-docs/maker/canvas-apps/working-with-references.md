...
1. На панели навигации слева выберите **данные**  > **сущности**.
...
1. Прокрутите вниз, пока не появится поле **Owner (владелец** ).
 ...
 > ![Owner поле в сущности учетной записи ](media/working-with-references/owner-field.png)
...
> [!div class="mx-imgBorder"]
> ![Accounts показан в элементе управления галереи ](media/working-with-references/accounts-gallery.png)
...
> [!div class="mx-imgBorder"]
> сущности ![Accounts, команды и пользователи в области данных ](media/working-with-references/accounts-datasources.png)
...
> [!div class="mx-imgBorder"]
> ![Accounts отображаются в элементе управления "Галерея" с отображаемым полем "владелец" ](media/working-with-references/accounts-displayowner.png)
...
> [!div class="mx-imgBorder"]
> ![Experimental переключить, чтобы включить управление ошибками на уровне формул ](media/working-with-references/accounts-iferror.png)
...
> [!div class="mx-imgBorder"]
> ![Added элемент управления "поле со списком" выше коллекции с свойством "элементы", имеющим значение "Пользователи" ](media/working-with-references/filter-insert-combobox.png)
...
> [!div class="mx-imgBorder"]
> Коллекция ![Filtered, основанная на значении, заданном в элементе управления "поле со списком" ](media/working-with-references/filter-accounts.png)
...
1. Наконец, задайте для свойства **Items** элемента управления **галереи** следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > Коллекция ![Filtered, в которой отображаются все записи или конкретный пользователь или команда ](media/working-with-references/filter-combobox.png)

С помощью этих изменений можно отобразить все записи или отфильтровать их в зависимости от пользователя или команды.

> [!div class="mx-imgBorder"]
> ![Animation отображение различных отфильтрованных результатов на основе списка переключателей и полей со списком ](media/working-with-references/filter-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![Filter по типу владельца с помощью типа ](media/working-with-references/filter-bytype.png)
...
1. В меню с многоточием выберите **Копировать эти элементы**.
    ...
    > [!div class="mx-imgBorder"]
    > ![Copy нескольких элементов управления с помощью представления в виде дерева ](media/working-with-references/patch-copy.png)

1. В том же меню выберите **Вставить**.

    > [!div class="mx-imgBorder"]
    > ![Paste нескольких элементов управления с помощью представления в виде дерева ](media/working-with-references/patch-paste.png)

1. Переместите скопированные элементы управления справа от коллекции.

    > [!div class="mx-imgBorder"]
    > ![Moved скопированные элементы управления справа от коллекции ](media/working-with-references/patch-position.png)
...
1. Выберите скопированный элемент управления **радио** , а затем измените следующие свойства:
    ...
    > [!div class="mx-imgBorder"]
    > ![Removed все, что выбрано из элемента управления радио ](media/working-with-references/patch-noall.png) 
...
1. Выберите видимый элемент управления **"поле со списком"** , а затем задайте для свойства **дефаултселектедитемс** следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > Свойство ![Default, заданное для поля со списком пользователей ](media/working-with-references/patch-default-users.png)
...
1. Выберите видимый элемент управления **"поле со списком"** для команд, а затем задайте для свойства **дефаултселектедитемс** следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > Свойство ![Default, заданное для поля со списком "команды" ](media/working-with-references/patch-default-teams.png)
...
1. Задайте в качестве значения свойства **OnSelect** кнопки следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > ![Formula задано для элемента управления "Кнопка" ](media/working-with-references/patch-button.png)
...
> [!div class="mx-imgBorder"]
> ![Animation отображение исправления владельца с помощью пользователя или команды ](media/working-with-references/patch-allthree.gif)
...
1. На вкладке **Свойства** в правой части экрана откройте список **источник данных** и выберите пункт **учетные записи**.
    ...
    > [!div class="mx-imgBorder"]
    > ![Form элемент управления, в котором отображаются дополнительные поля с пустыми значениями ](media/working-with-references/form-insert.png)  

1. Задайте для свойства **элемента** формы значение `Gallery1.Selected`.

    > [!div class="mx-imgBorder"]
    > ![Form элемент управления, содержащий дополнительные поля, заполненные из выбранного элемента в коллекции ](media/working-with-references/form-item.png)
...
1. В области **поля** нажмите кнопку с многоточием, а затем выберите **добавить настраиваемую карту**.
    ...
    > [!div class="mx-imgBorder"]
    > ![Command для добавления настраиваемой карты ](media/working-with-references/form-customcard.png)
...
1. При необходимости измените размер карточки, чтобы отобразить весь текст.
    ...
    > [!div class="mx-imgBorder"]
    > ![Inserted настраиваемая карточка, пустая ](media/working-with-references/form-inserted-customcard.png)
...
1. Вставьте элемент управления **Label** в настраиваемую карточку, а затем задайте для свойства **Text** метки формулу, которая использовалась в коллекции.
    ...
    > [!div class="mx-imgBorder"]
    > карточка ![Custom, показывающая поле "владелец" в элементе управления "метка" ](media/working-with-references/form-displayowner.png)
...
> [!div class="mx-imgBorder"]
> ![Animation отображение элемента управления формы, отвечающего на изменения в коллекции ](media/working-with-references/form-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![Contact сущность, показывающая поле "название компании" как тип данных клиента, который не является обязательным ](media/working-with-references/customer-companyname.png)
...
> [!div class="mx-imgBorder"]
> сущности ![Accounts, группы, пользователи и контакты в области данных ](media/working-with-references/customer-datasources.png)
...
| Свойство **элементов** переключателя фильтра | **[&nbsp; "все", &nbsp; "Пользователи", &nbsp; "команды" &nbsp;]** | **[&nbsp; "ALL", &nbsp; "Accounts", &nbsp; "Contacts" &nbsp;]** |
...
> [!div class="mx-imgBorder"]
> ![Update свойство Text элемента управления "метка подзаголовка" в коллекции ](media/working-with-references/customer-update.png)

С помощью этих изменений можно просмотреть и изменить поле **название компании** в сущности **Контакты** .

> [!div class="mx-imgBorder"]
> ![Animation, в котором показано, как выбор контакта изменяет другие элементы управления и форму ](media/working-with-references/customer-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![Data панель, в которой отображаются сущности Accounts, Teams, Users, Contacts и факсы ](media/working-with-references/faxes-datasources.png)
...
> [!div class="mx-imgBorder"]
> свойство текста ![Updated для элемента управления "подзаголовок" в отношении уточняющих запросов ](media/working-with-references/regarding-label.png)

После внесения этих изменений вы работаете с подстановкой «в **отношении** » так же, как у **владельца** и поиска **клиентов** .

> [!div class="mx-imgBorder"]
> ![Animation, показывающее, как при выборе элемента в коллекции изменяются другие элементы управления и форма ](media/working-with-references/regarding-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![Relationships сущности «факсы», которая отображается в отношении связей «многие к одному» ](media/working-with-references/activity-manytoone.png)
...
> [!div class="mx-imgBorder"]
> ![Relationships сущности учетной записи, показывающей отношение "один ко многим" ](media/working-with-references/activity-onetomany.png)
...
1. Добавьте еще один экран.
    ...
    > [!div class="mx-imgBorder"]
    > ![Insert пустого экрана ](media/working-with-references/activitypointer-newscreen.png)
...
1. На вкладке **Свойства** , расположенной в правой части экрана, задайте для **элементов** коллекции значение **учетные записи**.
    ...
    > [!div class="mx-imgBorder"]
    > ![Set элементы в учетные записи на панели свойств ](media/working-with-references/activitypointer-accounts.png)

1. Задайте для макета коллекции **заголовок**, а затем задайте для поля Название значение **имя учетной записи**.

    > [!div class="mx-imgBorder"]
    > ![Set макет в заголовок для элемента управления "Галерея" на панели "Свойства" ](media/working-with-references/activitypointer-account-name.png)
...
1. Задайте для свойства " **элементы** " новой коллекции значение `Gallery2.Selected.Faxes`.
    ...
    > [!div class="mx-imgBorder"]
    > ![Set свойство Items коллекции, показывающее факсы ](media/working-with-references/activitypointer-faxes.png)
...
1. Задайте макет коллекции как **заголовок и подзаголовок**, а затем задайте в поле Заголовок значение поля **Тема** (это может быть строчная **Тема**).
    ...
    > [!div class="mx-imgBorder"]
    > ![Set заголовок в поле Тема ](media/working-with-references/activitypointer-subject.png)

При выборе элемента в списке учетных записей список факсов показывает факсы только для этой учетной записи.

> [!div class="mx-imgBorder"]
> ![Animation отображение выбора в коллекции учетных записей, которая управляет списком факсов ](media/working-with-references/activitypointer-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![List сущностей, отображающих сущность действия ](media/working-with-references/activitypointer-entity.png)
...
Вы можете отобразить все действия, изменив только одну строку в предыдущем примере. Замените `Gallery2.Selected.Faxes` `Gallery2.Selected.Activities`.

> [!div class="mx-imgBorder"]
> Свойство ![Change элементов во второй коллекции с переходом от факсов к действиям ](media/working-with-references/activitypointer-gallery.png)
...
> [!div class="mx-imgBorder"]
> ![Data область, в которой показаны все сущности, необходимые для функции «функция» ](media/working-with-references/activity-datasources.png)
...
> [!div class="mx-imgBorder"]
> ![Set свойство текста в формулу для отображения сведений об факсах, телефонных вызовах и других действиях ](media/working-with-references/activitypointer-type.png)
...
> [!div class="mx-imgBorder"]
> ![Expanded свойство текста с дополнительными сведениями для телефонного звонка ](media/working-with-references/activitypointer-phonecall.png)
...
> [!div class="mx-imgBorder"]
> на ![Completed экране отображаются сведения о различных видах действий ](media/working-with-references/activitypointer-complete.png)
...
> [!div class="mx-imgBorder"]
> ![Enabling вложений и примечаний при создании сущности ](media/working-with-references/notes-entity.png)
...
> [!div class="mx-imgBorder"]
> сущность ![Account, показывающая связь с примечаниями с помощью связи "один ко многим" ](media/working-with-references/notes-relationships.png)
