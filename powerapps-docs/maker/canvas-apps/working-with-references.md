...
1. Откройте [этот сайт PowerApps](http://web.powerapps.com?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc).
...
Это поле подстановки можно ссылаться на запись из либо **команды** сущности или **пользователей** сущности. Не каждая запись в эти сущности имеет разрешение на быть **владельца**; проверьте поддерживаемые роли, в том случае, если вы столкнулись с проблемами.
...
> [!div class="mx-imgBorder"]
> ![Добавлен элемент управления полем со списком выше коллекции с свойства элементов для пользователей](media/working-with-references/filter-insert-combobox.png)
...
> [!div class="mx-imgBorder"]
> ![Отфильтрованные коллекции, в зависимости от значения, заданного в элементе управления полем со списком](media/working-with-references/filter-accounts.png)
...
1. Скопируйте и вставьте **поле со списком** управления, поместите копию прямо над исходной и затем настройте эти свойства для копирования:
    ...
    Приложение будет отображать только одно поле со списком одновременно, в зависимости от состояния радиосвязь. Так как они непосредственно над друг с другом, они отображаются быть тот же элемент управления, который изменяет его содержимое.
...
Если вы хотите тип владельца, можно использовать **IsType** функции, но он не еще делегировать нельзя.
...
1. Переместите скопированные элементы управления справа от элемента коллекции.
...
1. На **свойства** вкладка рядом с правой стороны экрана, откройте **источника данных** , а затем выберите **учетные записи**.
...
1. На **свойства** вкладка рядом с правой стороны экрана выберите **изменить поля**.
...
1. Измените размер карточки, при необходимости отображать весь текст.
...
Обработка **клиента** и **владельца** поля, практически одинаков, можно копировать приложение буквально (**файл** > **Сохранить как**, а затем укажите другое имя) и выполните эти простые замены:
...
Оба эти изменения, в том же формула, которая отображается в пользовательские карты в форме, а также, как в **текст** свойство элемента управления галереи метки:
...
> [!div class="mx-imgBorder"]
> ![Анимация, которая показывает, как выбор контакта изменяется, другие элементы управления и формы](media/working-with-references/customer-allthree.gif)
...
После внесения этих изменений, работе с **в отношении** подстановки точно так же, как это делалось **владельца** и **клиента** уточняющих запросов.

> [!div class="mx-imgBorder"]
> ![Анимация, которая показывает, как выбор элемента в коллекции изменяется, другие элементы управления и формы](media/working-with-references/regarding-allthree.gif)
...
Все сущности действий и задач действия сущности имеют неявную связь. Если вы измените фильтр на **все** в верхней части экрана, выберите **факсы** сущности, а затем выберите **связи** вкладке все сущности, которые могут быть целью  **В отношении** поиска отображаются.
...
1. На **свойства** вкладке рядом с правой стороны экрана, коллекции значение **элементы** для **учетные записи**.
...
1. Задайте новую коллекцию **элементы** свойства `Gallery2.Selected.Faxes`.
    ...
    > [!div class="mx-imgBorder"]
    > ![Задайте для свойства Items коллекции, показывающий факсов](media/working-with-references/activitypointer-faxes.png)
...
Если установить флажок для включения вложения, вы создадите **в отношении** взаимосвязь **заметки** сущности, как на этом рисунке показано для **учетные записи** сущности:
