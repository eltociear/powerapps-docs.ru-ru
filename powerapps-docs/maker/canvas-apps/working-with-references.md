...
1. Откройте [этот сайт Power Apps](https://make.powerapps.com?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc).
...
1. Прокрутите вниз, пока не появится поле **Owner (владелец** ).
 ...
 > ![поле владельца сущности учетной записи](media/working-with-references/owner-field.png)
...
> [!div class="mx-imgBorder"]
> ![учетных записей, команд и пользователей на панели «Данные»](media/working-with-references/accounts-datasources.png)
...
> [!div class="mx-imgBorder"]
> ![учетные записи, отображаемые в элементе управления "Галерея" с отображаемым полем "владелец"](media/working-with-references/accounts-displayowner.png)
...
> [!div class="mx-imgBorder"]
> ![экспериментальный коммутатор для включения управления ошибками на уровне формул](media/working-with-references/accounts-iferror.png)
...
> [!div class="mx-imgBorder"]
> ![добавлен элемент управления "поле со списком" над коллекцией, для свойства Items которых задано значение "Пользователи"](media/working-with-references/filter-insert-combobox.png)
...
> [!div class="mx-imgBorder"]
> ![фильтрованной коллекции на основе значений, заданных в элементе управления "поле со списком"](media/working-with-references/filter-accounts.png)
...
1. Наконец, задайте для свойства **Items** элемента управления **галереи** следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > ![отфильтрованной коллекции с отображением всех записей или определенного пользователя или команды](media/working-with-references/filter-combobox.png)

С помощью этих изменений можно отобразить все записи или отфильтровать их в зависимости от пользователя или команды.

> [!div class="mx-imgBorder"]
> ![анимация, отображающая различные отфильтрованные результаты на основе списка переключателей и полей со списком](media/working-with-references/filter-allthree.gif)
...
> [!div class="mx-imgBorder"]
> ![фильтровать по типу владельца с помощью](media/working-with-references/filter-bytype.png)
...
1. В меню с многоточием выберите **Копировать эти элементы**.
    ...
    > [!div class="mx-imgBorder"]
    > ![копии нескольких элементов управления с помощью представления в виде дерева](media/working-with-references/patch-copy.png)
...
1. Переместите скопированные элементы управления справа от коллекции.
    ...
    > [!div class="mx-imgBorder"]
    > ![перемещении скопированных элементов управления справа от коллекции](media/working-with-references/patch-position.png)
...
1. Выберите скопированный элемент управления **радио** , а затем измените следующие свойства:
    ...
    > [!div class="mx-imgBorder"]
    > ![все элементы, выбранные в переключателе, удалены](media/working-with-references/patch-noall.png) 
...
1. Выберите видимый элемент управления **"поле со списком"** , а затем задайте для свойства **дефаултселектедитемс** следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > ![набор свойств по умолчанию для поля со списком пользователей](media/working-with-references/patch-default-users.png)
...
1. Выберите видимый элемент управления **"поле со списком"** для команд, а затем задайте для свойства **дефаултселектедитемс** следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > ![набор свойств по умолчанию для поля со списком "команды"](media/working-with-references/patch-default-teams.png)
...
1. Задайте в качестве значения свойства **OnSelect** кнопки следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > ![формулу, заданную для элемента управления "Кнопка"](media/working-with-references/patch-button.png)
...
> [!div class="mx-imgBorder"]
> ![анимация, показывающая исправление владельца для пользователя или команды](media/working-with-references/patch-allthree.gif)
...
1. На вкладке **Свойства** в правой части экрана откройте список **источник данных** и выберите пункт **учетные записи**.
    ...
    > [!div class="mx-imgBorder"]
    > ![элемент управления формы, в котором отображаются дополнительные поля с пустыми значениями](media/working-with-references/form-insert.png)  

1. Задайте для свойства **элемента** формы значение `Gallery1.Selected`.

    > [!div class="mx-imgBorder"]
    > ![элемент управления формы, в котором отображаются дополнительные поля, заполненные выбранным элементом коллекции](media/working-with-references/form-item.png)
...
1. При необходимости измените размер карточки, чтобы отобразить весь текст.
    ...
    > [!div class="mx-imgBorder"]
    > ![вставлена настраиваемая карточка, пустая](media/working-with-references/form-inserted-customcard.png)
...
1. Вставьте элемент управления **Label** в настраиваемую карточку, а затем задайте для свойства **Text** метки формулу, которая использовалась в коллекции.
    ...
    > [!div class="mx-imgBorder"]
    > ![настраиваемой карте, показывающей поле "владелец" в элементе управления "метка"](media/working-with-references/form-displayowner.png)
...
> [!div class="mx-imgBorder"]
> ![учетных записей, групп, пользователей и контактов на панели «Данные»](media/working-with-references/customer-datasources.png)
...
> [!div class="mx-imgBorder"]
> ![обновление свойства текста элемента управления "метка подзаголовка" в коллекции](media/working-with-references/customer-update.png)
...
> [!div class="mx-imgBorder"]
> ![панели данных с сущностями учетные записи, группы, пользователи, контакты и факсы](media/working-with-references/faxes-datasources.png)
...
> [!div class="mx-imgBorder"]
> ![Обновлено свойство текста для элемента управления "подзаголовок" в отношении уточняющих запросов](media/working-with-references/regarding-label.png)
...
> [!div class="mx-imgBorder"]
> ![связи сущности «факсы», отображаемые в отношении связей «многие к одному»](media/working-with-references/activity-manytoone.png)
...
> [!div class="mx-imgBorder"]
> ![связи сущности учетной записи, отображаемой в отношении связей "один ко многим"](media/working-with-references/activity-onetomany.png)
...
1. На вкладке **Свойства** , расположенной в правой части экрана, задайте для **элементов** коллекции значение **учетные записи**.
    ...
    > [!div class="mx-imgBorder"]
    > ![задать элементы для учетных записей на панели свойств](media/working-with-references/activitypointer-accounts.png)

1. Задайте для макета коллекции **заголовок**, а затем задайте для поля Название значение **имя учетной записи**.

    > [!div class="mx-imgBorder"]
    > ![установить макет в заголовок для элемента управления "Галерея" на панели "Свойства"](media/working-with-references/activitypointer-account-name.png)
...
1. Задайте для свойства " **элементы** " новой коллекции значение `Gallery2.Selected.Faxes`.
    ...
    > [!div class="mx-imgBorder"]
    > ![задать свойство Items коллекции, в которой отображаются факсы](media/working-with-references/activitypointer-faxes.png)
...
1. Задайте макет коллекции как **заголовок и подзаголовок**, а затем задайте в поле Заголовок значение поля **Тема** (это может быть строчная **Тема**).
    ...
    > [!div class="mx-imgBorder"]
    > ![задать заголовок для поля Тема](media/working-with-references/activitypointer-subject.png)
...
> [!div class="mx-imgBorder"]
> ![изменить свойство Items во второй коллекции, изменив факсы на действия](media/working-with-references/activitypointer-gallery.png)
...
> [!div class="mx-imgBorder"]
> ![область данных, в которой отображаются все сущности, необходимые для функции «функция»](media/working-with-references/activity-datasources.png)
...
> [!div class="mx-imgBorder"]
> ![задать для свойства Text значение формула для отображения сведений об факсах, телефонных вызовах и других действиях](media/working-with-references/activitypointer-type.png)
...
> [!div class="mx-imgBorder"]
> на ![завершенном экране отображаются сведения о различных видах действий](media/working-with-references/activitypointer-complete.png)
...
> [!div class="mx-imgBorder"]
> сущность учетной записи ![, показывающая связь с примечаниями с помощью связи "один ко многим"](media/working-with-references/notes-relationships.png)
