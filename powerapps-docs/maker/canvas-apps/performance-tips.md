...
![Параллельные ClearCollect](./media/performance-tips/perfconcurrent2.png)  
...
Чтобы повысить производительность, экраны приложения загружаются в память только в том случае, когда они нужны. Эта оптимизация может возникают проблемы, если, например, загружается первый экран, и одну из его формул использует свойства элемента управления с экрана 2. Теперь экран 2 должен быть загружен для выполнения зависимости, прежде чем можно будет отобразить первый экран. Представьте, что экран 2 имеет зависимость на экране 3, который имеет еще одной зависимости на экране 4 и т. д. Эта цепочка зависимостей может привести к много экранов для загрузки.

По этой причине следует Избегайте формул зависимости между экранами. В некоторых случаях можно использовать глобальную переменную или коллекцию для обмена информацией между экранами.

Существует одно исключение. В предыдущем примере представьте, что единственным способом, чтобы отобразить экран 1, перейдя из экрана 2. Затем экран 2 будет уже загружены в память при экран 1 было необходимо загрузить. Дополнительных действий не требуется для удовлетворения зависимостей для экрана 2, поэтому не влияет на производительность.
...
## <a name="avoid-repeating-the-same-formula-in-multiple-places"></a>Не повторять та же формула в нескольких местах
Если несколько свойств выполнения одной и той же формулы (особенно в случае сложных), рассмотрите возможность установки его один раз, а затем ссылаясь на выходные данные первого свойства в последующие. Например, не устанавливайте **DisplayMode** свойства элементов управления A, B, C, D и E для одной и той же сложные формулы. Вместо этого задайте A **DisplayMode** сложных формулу, набора B **DisplayMode** свойство к результату A **DisplayMode** свойство, и так далее для C, D и E.

## <a name="enable-delayoutput-on-all-text-input-controls"></a>Включить DelayOutput на все элементы управления вводом, текст
При наличии нескольких формулах или правила, которые ссылаются на значение **ввода текста** , назначьте **DelayedOutput** свойства этого элемента управления значение true. **Текст** свойства этого элемента управления будет обновлен только в том случае, после перестали комбинаций клавиш в короткий промежуток времени. Формулы или правила не выполняются столько раз, и производительность приложения.

## <a name="avoid-using-formupdates-in-rules-and-formulas"></a>Избегайте использования Form.Updates правила и формулы
Если вы ссылаетесь значение входных данных пользователя в правиле или формулы с помощью **Form.Updates** переменной, он выполняет итерацию по карточки данных все формы и создает запись каждый раз. Чтобы сделать приложение более эффективным, сослаться на значение непосредственно из карточки данных или значение элемента управления.

## <a name="next-steps"></a>Дальнейшие действия
Просмотрите [стандарты кодирования](https://aka.ms/powerappscanvasguidelines) повышения производительности приложений и обеспечивая приложений проще было обслуживать.
