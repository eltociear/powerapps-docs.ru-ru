...
3. Выберите PowerApps.
    ...
    > [!NOTE]
    > Если у вас нет лицензии PowerApps, вы увидите синюю информационную панель со ссылкой для запуска пробной версии. При выборе для запуска пробной версии на новой вкладке появится новая вкладка, где вы будете подписаться на лицензию. По завершении вернитесь к портал Azure и обновите колонку, чтобы продолжить.
...
- Хранимая процедура [sp_set_firewall_rule](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-set-firewall-rule-azure-sql-database?view=azuresqldb-current) для базы данных SQL или правила брандмауэра SQL Server
...
PowerApps может подключиться к базе данных SQL, **разрешающей доступ к службе управления службами Azure** с помощью портал Azure. Чтобы настроить этот доступ, войдите в [портал Azure](https://portal.azure.com/) и перейдите на портале, чтобы **SQL Server**. Выберите **брандмауэры и виртуальные сети** и настройте элемент управления **Разрешить службам и ресурсам Azure доступ к этому серверу** **.** Нажмите кнопку **сохранить** , чтобы отправить изменения.
