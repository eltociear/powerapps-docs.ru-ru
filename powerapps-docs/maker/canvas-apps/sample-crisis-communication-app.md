...
| **Отображаемое название**| **Назначение** | **Description;** |
|-|-|-|
...
|-|-|-|
| Полное имя | ПолноеИмя | Имя контакта. |
...
Действие текущего **получения членов группы ограничено получением** 5000 пользователей, если вы используете лицензию Office для Power автоматизировать. Если у вас есть лицензия Premium и вы хотите распространить до 100000 пользователей, можно выполнить следующие действия, чтобы отправить больше пользователей.

1. Выберите меню **...** в карточке **получить членов группы** .

    ![Выберите... меню](media/sample-crisis-communication-app/40-Settings.png)

1. Нажмите **Параметры**.

1. Измените **значение поля порог** на 100 000.

    ![Установка поля порогового значения](media/sample-crisis-communication-app/41-Threshold.png)
...
1. В меню **файл** выберите **Параметры и настройки**, а затем выберите **Параметры источника данных**.
    ...
    ![Параметры источника данных](media/sample-crisis-communication-app/PBI-1-DataSourceSettings.PNG)
