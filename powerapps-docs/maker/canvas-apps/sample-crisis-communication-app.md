...
| Рейтинг | Рейтинг | Порядок вопроса в разделе часто задаваемых вопросов. |
...
### <a name="optional-sending-notifications-to-more-than-5000-users"></a>Необязательно: отправка уведомлений более чем 5000 пользователям

Действие текущего **получения членов группы ограничено получением** 5000 пользователей для лицензии Office Power автоматизиру. Даже для лицензии уровня "Премиум" вы можете ограничить пределы регулирования с помощью соединителя команд, если вы попытаетесь отправить уведомления слишком большому числу пользователей. Чтобы распространить среди других пользователей, можно преобразовать их в последовательность для отправки сообщения электронной почты в список рассылки.

1. Удалите следующие карточки: **Получение членов группы** и **Переключение предпочтительного параметра уведомления об отправке**:

    ![Удалить действия](media/sample-crisis-communication-app/36-delete-actions.png)

1. Добавьте новое действие.

1. Найдите и выберите **Отправить сообщение электронной почты (v2)** :

    ![Добавить отправить сообщение электронной почты](media/sample-crisis-communication-app/37-add-send-an-email.png)

1. В поле **Кому** введите имя группы рассылки.

1. В поле **Тема** нажмите кнопку **добавить динамическое значение** и добавьте поле **Title (заголовок** ) из поля **при публикации** карточки.

    ![Добавьте заголовок.](media/sample-crisis-communication-app/38-add-title.png)
...
Чтобы координировать свой ответ на критический момент, необходимо создать централизованную группу аварийного управления в командах и заполнить ее всей важной информацией. Эта группа должна быть предоставлена только группе Центральных ответов.
...
* **Как можно обновить приложение?**

    Если вы хотите обновить приложение, выполните действия, описанные в разделе [AKA.MS/CrisisCommunicationSolution](https://aka.ms/CrisisCommunicationSolution).
