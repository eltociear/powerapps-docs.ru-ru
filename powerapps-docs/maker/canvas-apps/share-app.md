...
> - Power Apps приложение Canvas гостевого доступа использует Azure B2B. Power Apps распознает гостей, указанные состояниями 1 – 4 в [документации по Azure B2B](https://docs.microsoft.com/azure/active-directory/b2b/user-properties). Power Apps не может распознать гостей, которые проходят проверку подлинности с помощью [прямой Федерации Azure AD](https://docs.microsoft.com/azure/active-directory/b2b/direct-federation). 
...
После того как вы поделитесь своим приложением для гостевого доступа, гости смогут обнаруживать приложения и получать к ним доступ из сообщения электронной почты, отправленного им в рамках общего доступа. Кроме того, можно предоставить URL-адрес приложения непосредственно гостевой виртуальной машине. Чтобы найти URL-адрес, перейдите в раздел [Power Apps](https://make.powerapps.com), выберите **приложения** из левой панели, **выберите приложение** и перейдите на вкладку **сведения** . URL-адрес приложения отображается в разделе **веб-ссылка**.
...
#### <a name="in-power-apps-mobile-how-does-a-guest-see-apps-in-the-guest-tenant"></a>Как гость видят приложения в гостевой клиенте в Power Apps?
В качестве гостевого пользователя откройте сообщение электронной почты, полученное при совместном использовании приложения в гостевой клиенте, и нажмите кнопку **открыть приложение** . Это относится как к Azure Active Directory, так и к пользователям учетных записей Майкрософт.   
...
| Планировщик                                           | Нет                                                                     |
| Plumsail формы                                    | Нет                                                                     |
| и просматривать их в Power BI                                          | Да                                                                    |
| Project Online                                    | Нет                                                                     |
