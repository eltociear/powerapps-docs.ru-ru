...
1. Укажите по имени или псевдониму пользователей или групп безопасности в Azure Active Directory, с которыми вы хотите предоставить общий доступ к приложению.
    ...
    - Вы можете поделиться приложением со списком псевдонимов, понятными именами или их сочетанием (например, **Мария петров &lt; @ no__t-2 >**), если элементы разделены точками с запятой. Если несколько человек имеют одно и то же имя, но разные псевдонимы, в список будет добавлен первый найденный пользователь. Всплывающая подсказка появляется, если имя или псевдоним уже имеют разрешение или не удается разрешить. 
...
1. Если приложение подключается к данным, для которых пользователям требуются разрешения на доступ, укажите их.
    ...
    > @no__t 0Assign роль безопасности @ no__t-1
...
1. Выберите роль или роли в Common Data Service, которые вы хотите назначить пользователю или группе безопасности в Azure AD, к которой вы хотите предоставить общий доступ к приложению.
     > [!div class="mx-imgBorder"] 
     > (media/share-app/cds-assign-security-role-list.png "Список ролей безопасности") ![списка ролей]безопасности
...
- Гостевой пользователь должен иметь лицензию PowerApps, назначенную одним из следующих клиентов:
...
1. Выберите **Новый гостевой пользователь** , чтобы добавить гостевых пользователей в Azure AD. Дополнительные сведения: [Краткое руководство: Добавьте нового пользователя Guest в Azure AD](/azure/active-directory/b2b/b2b-quickstart-add-guest-users-portal).
    > [!div class="mx-imgBorder"] 
...
3. Предоставьте общий доступ к приложению Canvas. 
    1. Вход в https://make.powerapps.com  
    ...
    3. Введите адрес электронной почты гостевого пользователя из клиента Azure AD. Дополнительные сведения: [Что такое доступ гостевых пользователей в Azure AD B2B?](/azure/active-directory/b2b/what-is-b2b)
          > [!div class="mx-imgBorder"] 
          > ![Поделиться с]гостевой(media/share-app/guest_access_doc_2.png "общей папкой с гостевой службой")
...
> [!div class="mx-imgBorder"]  
> ![Гости получают общий доступ к приложению электронная почта](media/share-app/guest_access_doc_4.png "гости получение электронная почта для приложения")
...
> [!div class="mx-imgBorder"]  
> ![Внедрение приложения Canvas в SharePoint для](media/share-app/guest_access_doc_5.PNG "приложения внедрения холста \"гости\" в SharePoint для гостей")

При внедрении приложения Canvas в SharePoint с помощью HTML-тега iFrame сослаться на приложение, используя полный веб-URL. Чтобы найти URL-адрес, перейдите в http://make.powerapps.com, выберите приложение, перейдите на вкладку " **сведения** ", и URL-адрес отобразится в разделе " **веб-ссылка**".

> [!div class="mx-imgBorder"]  
> ![]Сведения о приложении холста сведения(media/share-app/guest_access_doc_6.PNG "о приложении Canvas")
...
InTune применяет политики для домашнего клиента пользователя. Например, если Alice@Contoso.com совместно использует приложение с Vikram@Fabrikam.com, InTune продолжит применять политики Fabrikam.com на устройстве Виркам независимо от того, какой PowerApps он работает.
