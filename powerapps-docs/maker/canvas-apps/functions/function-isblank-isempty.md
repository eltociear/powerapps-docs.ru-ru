...
*Пустая строка* ссылается на строку, которая не содержит символов.  Функция [ **Len** ](function-len.md) возвращает ноль для такой строки, и она может быть написана в формулах как две двойные кавычки с nothing в диапазоне от `""`.  Некоторые элементы управления и источники данных используют пустую строку для обозначения условия "без значения".  Чтобы упростить создание приложений, функции IsEmpty и **объединения** проверяют как *пустые* значения, **так и пустые** строки.    
...
`Coalesce( value1, value2 )` является более кратким эквивалентом `If( Not IsBlank( value1 ), value1, Not IsBlank( value2 ), value2 )` и не требует дважды вычислять **Значение1** и **value2** .  Функция [ **If** ](function-if.md) возвращает значение *Blank* , если в этом случае отсутствует формула "else".
...
* *Значение* — обязательный аргумент. Значение для проверки на *пустое* значение или пустую строку.
...
| --- | --- | --- |
| **Объединение (&nbsp;Blank (), &nbsp;1 @ no__t-3)** |Проверяет возвращаемое функцией **Blank** значение (всегда *пустое*). Поскольку первый аргумент *пуст*, вычисление продолжится со следующим аргументом, пока не будет обнаружено*непустое* значение и непустая строка. |**1** |
...
| Формула | Описание | Возвращаемый результат |
| --- | --- | --- |
| **Blank (&nbsp;Blank () &nbsp;)** |Проверяет возвращаемое функцией **Blank** значение (всегда *пустое*). |**true** |
...
2. Задайте для свойства кнопки **[OnSelect](../controls/properties-core.md)** эту формулу:
    ...
    **Collect (Ицекреам, {версия: "Strawberry", количество: 300}, {версия: "Шоколад", количество: 100})**
3. Просмотрите приложение, нажмите добавленную кнопку, а затем закройте режим предварительного просмотра.  
