...
| **Объединение ("")** | Проверяет первый аргумент, который является пустой строкой. Поскольку первый аргумент представляет собой пустую строку, и больше нет аргументов, функция возвращает *пустое*значение.   |*пусто* |
