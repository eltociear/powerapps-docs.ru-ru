...
Глобальные переменные доступны во всем приложении и на всех экранах. Это самые простые переменные, которые подойдут для большинства ситуаций. Существуют также переменные контекста, которые применяются к одному экрану, и коллекции, которые позволяют изменять таблицы на уровне строк. Дополнительные сведения об этих других параметрах см. в статье сведения о [переменных](../working-with-variables.md).

Служба PowerApps основывается на формулах, которые автоматически вычисляются при взаимодействии пользователя с приложением. Все формулы, зависящие от переменной, будут автоматически обновляться при изменении. Однако переменная не будет автоматически обновляться при изменении значения формулы, используемой в функции **Set** . Для этого необходимо, чтобы разработчик приложения вручную обновляет переменную, которая может быть подвержена ошибкам и сложнее для понимания другими пользователями. Прежде чем использовать переменную, ознакомьтесь со [сведениями о переменных](../working-with-variables.md).

## <a name="description"></a>Описание
Глобальные переменные создаются неявно с помощью функции **Set**. Явное объявление не требуется. Если удалить все функции **набора** для глобальной переменной, эта глобальная переменная перестанет существовать. Чтобы очистить переменную, присвойте ее значение результату [функции **Blank** ](function-isblank-isempty.md).

Значения переменных, определения и использование можно просмотреть с помощью представления переменные в меню **файл** в PowerApps Studio.
