...
ms.locfileid: "65986335"
---
# <a name="astype-and-istype-functions-in-canvas-apps"></a>Функции AsType и IsType в приложения на основе холста

Проверяет ссылку на запись для определенного типа сущности (**IsType**) и считает ссылки определенного типа (**AsType**).
...
**IsType** функция проверяет ли ссылка в записи для определенного типа сущности. Функция возвращает логическое значение TRUE или FALSE.
...
Если ссылка на запись является *пустой*, **IsType** возвращает значение FALSE, и **AsType** возвращает *пустой*. Все поля *пустой* запись будет *пустой*.
...
- *RecordReference* — обязательный. Ссылка записи, часто полем поиска, которое может указывать на запись в любом из нескольких сущностей.
- *EntityType* — обязательный. Конкретная сущность, к которому следует привести запись.
...
1. Вставить **коллекции** управления **пустая, вертикальная** ориентации.
...
1. На **свойства** вкладке рядом с правой стороны экрана, коллекции значение **элементы** свойства **контакты**.
