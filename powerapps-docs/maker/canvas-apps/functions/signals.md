...
| **App.SizeBreakpoints** | Таблица с одним столбцом чисел, размер экрана разделения диапазонов адресов, [ **Screen.Size** ](../controls/control-screen.md) возвращает. В этой таблице могут изменяться для настройки точки останова, что все приложения экраны использования.

**Приложения** объект также имеет [формуле поведения](../working-with-formulas-in-depth.md) , которые можно задать.

| Свойство  | Описание |
| --- | --- |
| **App.OnStart** | Поведение приложения, когда пользователь запускает его. Разработчики часто используют это свойство для извлечения и кэширования данных в коллекции с **[собирать](function-clear-collect-clearcollect.md)** функцию, задать переменные с **[задать](function-set.md)** работать и перейдите на начальный экран с **[Navigate](function-navigate.md)** функции. Эта формула вычисляется перед появлением первого экрана. Экран не загружается, поэтому нельзя задать переменные контекста с **[UpdateContext](function-updatecontext.md)** функции. Тем не менее, можно передавать переменные контекста с **Navigate** функции. |
