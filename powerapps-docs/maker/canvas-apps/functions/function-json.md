...
| **Option @ no__t-1set** | Числовое значение набора параметров, а не метки, используемой для вывода. Числовое значение используется, так как оно не зависит от языка.  | `1001` |
...
| **Два @ no__t-1option** | Логическое значение двух параметров ( *true* или *false*), а не метка, используемая для вывода. Логическое значение используется, так как оно не зависит от языка. | `false` |
...
1. Нажмите кнопку, удерживая клавишу ALT.
    ...
    Эту коллекцию также можно показать, выбрав **файл** > **Параметры приложения** > **Дополнительные параметры** > **включить представление результатов панели формул**, выбрав имя коллекции в строке формул, а затем выбрав элемент Стрелка вниз рядом с именем коллекции в строке формул.

    > [!div class="mx-imgBorder"]
    > ![Collection в представлении результатов в строке формул @ no__t-1
