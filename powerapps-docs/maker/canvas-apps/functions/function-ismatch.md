...
|----|----|----|
| *именованное&#8209;подсовпадение&#8209;или подзапросы* | Текст | Каждое именованное вложенное соответствие будет иметь свой собственный столбец. Создание именованного подзапроса с помощью **(?&lt; *имя* &gt;** ... **)** в регулярном выражении. Если именованное вложенное совпадение имеет то же имя, что и один из заранее определенных столбцов (см. ниже), то это имеет приоритет, и создается предупреждение. Чтобы избежать этого предупреждения, переименуйте подстроку. |
...
В языке шаблона некоторые символы зарезервированы для особых целей. Чтобы использовать эти символы, добавьте префикс символа **\\** (обратная косая черта), чтобы указать, что символ должен быть создан буквально, или используйте один из стандартных шаблонов, описанных далее в этом разделе. В следующей таблице перечислены специальные символы.
...
Предопределенные шаблоны предоставляют простой способ сопоставления либо одного из наборов символов, либо последовательности из нескольких символов. [ **Оператор&** объединения строк](operators.md) используется для объединения собственных текстовых строк с элементами перечисления **Match** :
...
- Именованные обратные ссылки  **\\:&lt;*имя* k&gt;**
...
**Соответствие** ( *Текст*, *шаблон* [, *Параметры* ])
...
| Формула | Описание | Результат |
|--------|------------|-----------|
| `Match( "Bob Jones <bob.jones@contoso.com>", "<(?<email>" & Match.Email & ")>"` | Извлекает только часть электронной почты контактной информации.  | {<br>адрес электронной&nbsp;почтыbob.jones@contoso.com: "",<br>Фуллматч:&nbsp;"&lt;>",bob.jones@contoso.com<br>Подсовпадений:&nbsp;[&nbsp;"bob.jones@contoso.com"&nbsp;],<br>Стартматч: 11<br>}  
...
| `Match( pangram, "\b\wo\w\b\s\*(?<between>\w.+\w)\s\*\b\wo\w\b" )` | Соответствует всем символам между "Fox" и "Dog". | {<br>между:&nbsp;&nbsp;"переходит&nbsp;наотложенный",&nbsp;<br>Фуллматч:&nbsp;"Fox&nbsp;переходит&nbsp;на&nbsp;отложенную&nbsp;собака",&nbsp;<br>Подсовпадений: ["переходит на отложенную"],<br>Стартматч: 17<br> } |
