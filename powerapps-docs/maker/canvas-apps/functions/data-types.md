...
|-----------|-------------|---------|
| **Логическая** | Значение *true* или *false* .  Может использоваться непосредственно в операторе **If**, **Filter** и других функциях без сравнения.  | *true* |
| **Цвет** | Спецификация цвета, включая альфа-канал. | **Color.Red**<br>**Колорвалуе ("#102030")**<br>**RGBA (255, 128, 0, 0,5)** |
| **Денежный формат** | Значение валюты, хранящееся в числе с плавающей запятой. Значения валют совпадают с числовыми значениями с параметрами форматирования валюты.  | **123**<br>**4,56** |
...
| **Ссылка** | Текстовая строка, содержащая гиперссылку. | **"https://powerapps.microsoft.com"** |
...
| **Запись** | Запись значений данных. Этот составной тип данных содержит экземпляры других типов данных, перечисленных в этом разделе. Дополнительные сведения: [Работа с таблицами](../working-with-tables.md). | **{Company: "Northwind Trader",<br>персонал: 35, <br>неприбыльности: false}** |
...
### <a name="embedded-text"></a>Внедренный текст

Встроенные текстовые строки в формуле заключаются в двойные кавычки.  Чтобы представить одинарную двойную кавычку в текстовой строке, используйте две двойные кавычки.  Например, с помощью следующей формулы в свойстве **OnSelect** элемента управления [**Button**](../controls/control-button.md) :

```powerapps-dot
Notify( "Jane said ""Hello, World!""" )
```

приводит к появления баннера при нажатии кнопки, где опущены первая и последняя двойная кавычки (так как они разделяют текстовую строку) и повторяющиеся двойные кавычки вокруг **Hello, World!** заменяются одной двойной кавычкой:

![всплывающее уведомление с сообщением Мария сказал «Hello, World»](media/data-types/literal-string.png)

Одинарные кавычки не используются для [имен идентификаторов](operators.md#identifier-names) , содержащих специальные символы и не имеющие значимости в текстовой строке.  
...
Универсальный код ресурса (URI) используется для ссылки на изображение или на другой файл мультимедиа, хранящийся в базе данных. Таким образом, приложение не будет получать фактические данные, пока не будет действительно требуется. Например, вложение в Common Data Service сущность может возвращать **"appres://DataSources/Contacts/Table/..."** , как показано в примере с камерой. это изображение можно отобразить, задав для свойства **Image** элемента управления Image эту ссылку, которая извлекает двоичные данные.
...
- **Локальная пользователь**: эти значения хранятся в [формате UTC (время в формате всемирного координированного времени)](https://en.wikipedia.org/wiki/Coordinated_Universal_Time), но часовой пояс пользователя приложения влияет на отображение этих значений в приложении и о том, как они указываются пользователем приложения. В качестве примера один и тот же момент по-разному отображается для пользователя в Канаде, чем для пользователя в Японии.
- **Независимо**от часового пояса. приложение показывает эти значения одинаковым образом, и пользователь приложения указывает их одинаковым образом независимо от часового пояса. Этот же момент аналогичен пользователю в Канаде, как это делается для пользователя в Японии. Авторы приложений, которые не хотят, чтобы их приложения выполнялись в разных часовых поясах, используют эти значения, так как они проще всего использовать.
...
|--------------------------|------------------------------|------------------------------|
| **Локальная пользовательская** | Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>4:00 AM | Суббота,&nbsp;может&nbsp;18,&nbsp;2019<br>21:00 | Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>8:00 AM |
| **Независимо от часового пояса** | Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>4:00 AM | Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>4:00 AM | Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>4:00 AM | 
...
|--------------------------|------------------------------|------------------------------|
| **Локальная пользовательская** | Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>4:00 AM | Суббота,&nbsp;может&nbsp;18,&nbsp;2019<br>21:00 | 1 558 238 400 000<br> (Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>4:00 AM (UTC) |
| **Независимо от часового пояса** | Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>4:00 AM | Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>4:00 AM |1 558 263 600 000<br> (Воскресенье,&nbsp;может&nbsp;19,&nbsp;2019<br>11:00 AM (UTC) |
...
Если требуется время UNIX в значении **даты** для дальнейших вычислений или отображение в Power Apps, используйте следующую формулу:
