---
title: Использование Cognitive Services в Power Apps | Документация Майкрософт
description: Создание базового приложения Canvas, использующего API анализа текста Cognitive Services Azure для анализа текста.
...
> [!NOTE]
> Если вы не знакомы с созданием приложений в Power Apps, мы рекомендуем ознакомиться со статьей [Создание приложения с нуля](get-started-create-from-blank.md) , прежде чем углубляться в эту статью.

## <a name="introduction-to-azure-cognitive-services"></a>Введение в Azure Cognitive Services
Cognitive Services Azure — это набор API-интерфейсов, пакетов SDK и служб, позволяющих сделать приложения более интеллектуальными, привлекательными и обнаруживаемыми. Эти службы позволяют легко добавлять в приложения такие интеллектуальные функции, как обнаружение эмоций и видео, распознавание лиц, речи и визуальных образов, а также понимание речи и языка.
...
3. На странице отображаются отформатированные результаты на вкладке " **анализируемый текст** " и ответ JSON на **JSON** Tab. [JSON](https://json.org/) — это способ представления данных — в этом случае данные, возвращаемые API анализа текста.
...
2. На [этой странице](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesTextAnalytics)введите сведения для API анализа текста, как показано на этом рисунке. Выберите ценовую категорию **F0** (бесплатно).
...
2. В средней области в Power Apps Studio выберите **подключиться к данным**.
...
* Для каждого вызова укажите соответствующие параметры:
  ...
  * В **фраз ()** и **детектсентимент ()** **язык** жестко закодирован как "en", но этот параметр можно передать на основе некоторой логики в приложении. Например, можно сначала определить язык пользователя с помощью метода **DetectLanguage()** , а затем задать это значение в нашем параметре.
