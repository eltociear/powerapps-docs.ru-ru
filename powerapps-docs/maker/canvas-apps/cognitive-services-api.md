---
title: Использование служб Cognitive Services с Power Apps | Документация Майкрософт
description: Создание простого приложения на основе холста, использующего API анализа текста Azure Cognitive Services.
...
3. Отформатированные результаты отобразятся на вкладке **Анализируемый текст**, а ответ в формате JSON — на вкладке **JSON** этой страницы. [JSON](https://json.org/) — это способ представления данных. В нашем случае это данные, возвращаемые API-интерфейсом анализа текста.
...
2. На [этой странице](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesTextAnalytics) введите сведения для API анализа текста, как показано на рисунке. Выберите ценовую категорию **F0** (бесплатно).
...
Теперь API анализа текста готов и запущен. Вы можете подключиться к нему через Power Apps и создать приложение, которое вызывает этот API. У этого приложения будет один экран и такие же функциональные возможности, как у демоверсии на странице API анализа текста. Приступим к созданию.

### <a name="create-the-app-and-add-a-connection"></a>Создание приложения и добавление подключения
Сначала создайте пустое приложение для телефона и добавьте подключение с помощью соединителя **Текстовая аналитика**. Дополнительные сведения об этих задачах см. в статьях [Создание приложения с нуля](get-started-create-from-blank.md) и [Управление подключениями в Power Apps](add-manage-connections.md).
...
2. В средней области Power Apps Studio выберите вариант **подключения к данным**.
...
2. Данные, полученные от API, сохраняются в трех разных [коллекциях](working-with-variables.md#use-a-collection): **languageCollect**, **sentimentCollect** и **phrasesCollect**.
...
* Для каждого вызова укажите соответствующие параметры:
  ...
  * В методах **KeyPhrases()** и **DetectSentiment()** для параметра **language** жестко задано значение "en" (английский), однако вы можете изменять его в приложении на основе каких-либо условий. Например, можно сначала определить язык пользователя с помощью метода **DetectLanguage()** , а затем задать это значение в нашем параметре.
