...
description: Создайте базовый холст приложение, которое использует API Azure Cognitive Services Text Analytics для анализа текста.
author: gregli-msft
manager: kvivek
...
2. В [эту страницу](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesTextAnalytics), введите сведения для API анализа текста, как на этом изображении показаны. Выберите ценовую категорию **F0** (бесплатно).
...
* Для каждого вызова укажите соответствующие параметры:
  ...
  * В **KeyPhrases()** и **DetectSentiment()**, **языка** задано жестко, как «en», но можно передать этот параметр, на основе определенной логики в приложении. Например, можно сначала определить язык пользователя с помощью метода **DetectLanguage()**, а затем задать это значение в нашем параметре.
