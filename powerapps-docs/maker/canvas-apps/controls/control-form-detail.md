...
Можно также задать свойство **Item** формы с помощью элемента управления **Drop down**, как описано в разделе [Отображение, изменение или добавление записи](../add-form.md), или с помощью функции, такой как **Lookup** или **First**. Например, можно задать **элемент** значение либо из этих формул для отображения записи Fabrikam в **учетные записи** сущности в Common Data Service:
...
| **FormMode.New** |Пользователь может создать запись с помощью формы. Значения в элементы управления формы предварительно заполняются значениями по умолчанию для записи источника данных. При успешном выполнении функции **[SubmitForm](../functions/function-form.md)** создается запись. |
| **FormMode.View** |Пользователь может просматривать запись с помощью формы. Значения в элементы управления формы предварительно заполняются значениями по умолчанию для записи источника данных. |
...
* Это свойство возвращает запись из значений.  Например, если элемент управления формы содержит карточки для **имя** и **количество** поля и значения **[обновление](control-card.md)** свойства для этих карточек возвращают «Мини-приложение» и 10 соответственно, а затем **обновления** возвращает свойства для элемента управления формы **{имя: «Мини-приложение», Quantity: 10 }**.
