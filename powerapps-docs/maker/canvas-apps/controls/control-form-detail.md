...
Можно также задать свойство **Item** формы с помощью элемента управления **Drop down**, как описано в разделе [Отображение, изменение или добавление записи](../add-form.md), или с помощью функции, такой как **Lookup** или **First**. Например, можно задать для свойства **Item** одну из этих формул, чтобы отобразить запись Fabrikam в сущности **accounts** в Common Data Service:
...
| **FormMode.New** |Пользователь может создать запись с помощью формы. Значения в элементах управления формы предварительно заполнены значениями по умолчанию для записи источника данных. Если функция **[субмитформ](../functions/function-form.md)** выполняется успешно, создается запись. |
| **FormMode.View** |Пользователь может просматривать запись с помощью формы. Значения в элементах управления формы предварительно заполнены значениями по умолчанию для записи источника данных. |
...
* Это свойство возвращает запись из значений.  Например, если элемент управления формы содержит элементы управления карточками для полей **имя** и **количество** , а значения свойств **[обновления](control-card.md)** для этих карточек возвращают "Widget" и 10 соответственно, то свойство " **обновления** " для элемента управления формы возвратит **{Name: "Мини-приложение"; количество: 10}** .
