...
**[OnChange](properties-core.md)** — как приложение реагирует, когда пользователь изменяет значение элемента управления. 

Разница между **OnChange** принудительным включением и изменением в рамках **одного и того**же действия пользователя, если *Нажатие кнопки* пользователя приводит к изменению. В этом случае **перед** изменением активируется триггер OnSelect.
