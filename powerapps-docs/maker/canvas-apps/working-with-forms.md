...
description: В Power Apps Добавьте форму в приложение Canvas, чтобы можно было получать и отображать информацию из источника данных.
author: gregli-msft
manager: kvivek
...
| Действие | Элемент управления | Description |
| --- | --- | --- |
...
На предыдущем рисунке для выбранной карты отображается поле **AssetID** и элемент управления **[Текстовое поле](controls/control-text-input.md)** , чтобы пользователь мог изменить значение этого поля. (Напротив, экран сведений показывает то же поле в элементе управления **[Label](controls/control-text-box.md)** , которое доступно только для чтения.) Элемент управления **[вводом текста](controls/control-text-input.md)** имеет свойство **[по умолчанию](controls/properties-core.md)** , для которого задано значение **Parent. Default**. Если пользователь создал запись вместо того, чтобы отредактировать ее, этот элемент управления будет отображать начальное значение, чтобы пользователь мог изменить его для новой записи.
...
Зная, как Power Apps создает приложение, вы можете создать его самостоятельно, используя те же стандартные блоки и формулы, которые обсуждались ранее в этом разделе.
...
> [!NOTE]
> При использовании списка SharePoint или таблицы Excel, содержащей имена столбцов с пробелами в качестве источника данных, Power Apps заменит пробелы на **"\_x0020\_"** . Например, **"имя столбца"** в SharePoint или Excel будет отображаться как **"Column_x0020_Name"** в Power Apps, если оно отображается в макете данных или используется в формуле.
...
В приложении, которое создает приложение Power Apps из данных, свойству **[автовысоты](controls/control-text-box.md)** этого элемента управления присваивается *значение true* , чтобы не использовать пробелы при отсутствии ошибок. Свойства **[Height](controls/properties-size-location.md)** и **[Y](controls/properties-size-location.md)** элемента управления **[Форма редактирования](controls/control-form-detail.md)** также динамически корректируются с учетом увеличения элемента управления при появлении ошибки. Чтобы получить дополнительные сведения, создайте приложение на основе существующих данных и проверьте эти свойства. Элемент управления текстового поля для ошибок очень короткий, если ошибки не произошло, иногда, чтобы выбрать этот элемент управления, необходимо открыть представление **Advanced** (на вкладке **View**).
...
В приложении, созданном на основе данных, мы не будем обсуждать два элемента управления в верхней части экрана обзора. С помощью этих элементов управления пользователь может найти одну или несколько записей, отсортировать список записей в восходящем или нисходящем порядке, или сделать и то и другое.
