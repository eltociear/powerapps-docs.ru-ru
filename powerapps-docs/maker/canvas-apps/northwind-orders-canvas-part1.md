...
1. Присвойте приложению имя, которое вам нравится, а затем выберите **создать**.
    ...
    Откроется приложение Power Apps Studio, в котором можно добавить к приложению источники данных и элементы управления:
...
1. Введите **Orders** в поле поиска:

    > [!div class="mx-imgBorder"]
    > ![список подключений](media/northwind-orders-canvas-part1/datasource-02.png)

1. Выберите источник данных **Orders** для использования в приложении:
...
1. Как только мы будем работать с экранами и элементами управления, в Power Apps Studio переключитесь обратно в **представление в виде дерева** в левой части, нажав кнопку с тремя значками квадратов с накоплением. Вы можете в любое время вернуться к **источникам данных** , нажав на значок цилиндра.
...
1. На вкладке **Вставка** выберите **коллекция** > **пустой вертикальный** , чтобы добавить элемент управления из [**коллекции**](controls/control-gallery.md) , в котором будут показаны заказы.
    ...
    Элемент управления будет помещен на холст, и появится диалоговое окно с вопросом, к какому источнику данных подключаться.  
...
1. Мы можем напрямую подключить его к **заказам** , но вместо этого нам нужно управлять порядком сортировки коллекции.  Игнорируйте диалоговое окно "вылет" и в строке формул задайте в качестве значения свойства " **Items** " коллекции следующую формулу:
    ...
    > [!div class="mx-imgBorder"]
    > ![свойство "задать элементы" коллекции](media/northwind-orders-canvas-part1/orders-02b.png)

1. Через несколько секунд представление результатов появится под строкой формул.  Растяните стрелку слева, чтобы увидеть результат нашей формулы.  Прокрутите страницу вправо, чтобы просмотреть столбец **номер заказа** и убедиться, что он отсортирован нужным образом (от самого верхнего к меньшему).  

    > [!div class="mx-imgBorder"]
    > ![свойство "задать элементы" коллекции](media/northwind-orders-canvas-part1/orders-02c.png)
...
1. Выберите **изменить** (рядом с **полями**) на вкладке **Свойства** рядом с правой границей.

    > [!div class="mx-imgBorder"]
    > ![выбрать](media/northwind-orders-canvas-part1/orders-04b.png) макета
...
1. Задайте для свойства **Text** новой метки следующее выражение:
    ...
    В формулах необходимо окружить **состояние заказа** одинарными кавычками, так как оно содержит пробел. Однако это имя функционирует так же, как и любое другое имя в Power Apps, такое как **клиент** или **компания**.
...
1. В строке формул задайте для свойства **Цвет** метки состояние значение эта формула:
    ...
    Power Apps предотвращает создание формулы, основанной на жестко запрограммированной строке для каждого параметра в наборе, так как такие формулы могут привести к неуместным результатам, если имена параметров локализованы. Вместо этого функция **switch** определяет цвет на основе любой строки, отображаемой в метке на основе параметров пользователя.
