...
ms.locfileid: "74672998"
---
# <a name="create-a-rule-in-canvas-apps"></a>Создание правила в приложениях холста
Создавайте правила, чтобы приложение автоматически изменялось на основе заданных критериев. Например, отображать элементы списка красным, желтым или зеленым цветом в зависимости от их состояния или показывать кнопку утверждения только для определенных пользователей (например, менеджеров). Вы можете добавлять правила для различных элементов управления. В этой статье вы добавите правило для изменения цвета текста в элементе управления **Метка**, если значение элемента управления **Ползунок** больше 70.

> [!IMPORTANT]
> Начиная с 14 октября 2019 г. функции правил в приложениях Canvas не рекомендуются. Дополнительные сведения: [блог: нерекомендуемая функциональность правил холста](https://powerapps.microsoft.com/blog/canvas-rules-feature-deprecation/).
...
Действия позволяют определить, что должно произойти после применения правила. Power Apps может автоматически создавать действия в зависимости от изменений, внесенных в элементы управления.

1. Нажмите кнопку **Определить действия**.

    ![Определение действий](./media/working-with-rules/rule-define-actions.png)

1. В диалоговом окне подтверждения нажмите кнопку **"переключиться** ", чтобы Power Apps захватывает следующее изменение или изменения в виде одного или нескольких действий.
