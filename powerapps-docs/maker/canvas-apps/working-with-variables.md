...
| Монитор | Description |
|----|----|
| <style>img {max-width: None}</style> ![App с элементом управления вводом текста, меткой и двумя кнопками ](media/working-with-variables/button-changes-state-1.png) | При запуске приложения итоговая сумма равна 0.<br><br>Красная точка представляет палец пользователя в поле ввода текста, где пользователь вводит **77**. |
