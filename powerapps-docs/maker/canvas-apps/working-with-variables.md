...
Если вы использовали другое средство программирования, например Visual Basic или JavaScript, вы можете спросить: **Где находятся переменные?** PowerApps немного отличается и требует иного подхода. Вместо того чтобы обращаться к переменной при создании приложения Canvas, спросите себя: **Что делать в Excel?**
...
![Анимация вычисления суммы двух чисел в PowerApps](media/working-with-variables/recalc2.gif)
...
| Монитор | Описание |
|----|----|
| <style>img {max-width: None}</style> ![App с элементом управления вводом текста, меткой и двумя кнопками @ no__t-2 | При запуске приложения итоговая сумма равна 0.<br><br>Красная точка представляет палец пользователя в поле ввода текста, где пользователь вводит **77**. |
...
3. Чтобы нарастающий итог обновлялся, когда пользователь нажимает кнопку **Добавить**, задайте следующую формулу в качестве значения свойства **[OnSelect](controls/properties-core.md)** :
    ...
    **Set (Руннингтотал, Руннингтотал + TextInput1)**
    ...
    Только эта формула устанавливает **руннингтотал** как глобальную переменную, которая содержит число из-за оператора **@no__t 2** . Вы можете ссылаться на **руннингтотал** в любом месте приложения. Когда пользователь открывает это приложение, **руннингтотал** имеет *пустое*начальное значение.
...
PowerApps имеет три типа переменных:

| Тип переменных | Область действия | Описание | Функции, устанавливающие |
| --- | --- | --- | --- |
...
* Переменные контекста обновляются записями. В других средствах программирования для присвоения значений обычно используется оператор «=», например «x = 1». Для переменных контекста используйте **{x: 1}** . При использовании переменной контекста используйте ее имя напрямую без синтаксиса записи.
...
3. Чтобы нарастающий итог обновлялся, когда пользователь нажимает кнопку **Добавить**, задайте следующую формулу в качестве значения свойства **[OnSelect](controls/properties-core.md)** :
    ...
    @no__t 0UpdateContext ({Руннингтотал: Руннингтотал + TextInput1}) **

    Только эта формула устанавливает **руннингтотал** в качестве переменной контекста, содержащей число, из-за оператора **@no__t 2** . Вы можете ссылаться на **руннингтотал** в любом месте этого экрана. Когда пользователь открывает это приложение, **руннингтотал** имеет *пустое*начальное значение.
...
4. Чтобы нарастающий итог становился равным **0**, когда пользователь нажимает кнопку **Очистить**, задайте в качестве значения его свойства **[OnSelect](controls/properties-core.md)** следующую формулу:
    ...
    @no__t 0UpdateContext ({Руннингтотал: 0}) **

    Опять же, **[упдатеконтекст](functions/function-updatecontext.md)** используется с формулой **UpdateContext ({руннингтотал: 0})** .
...
> [!NOTE]
> Функции **SaveData** и **LoadData** в powerapps Mobile, но не PowerApps Studio или в веб-проигрывателе для PowerApps.