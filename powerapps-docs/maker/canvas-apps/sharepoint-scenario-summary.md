...
1. Когда вы добавляете элемент на шаге 1, запускается поток, который отправляет письмо для утверждения. Ниже показана папка "Входящие" учетной записи электронной почты утверждающего.
   
    ![Письмо с запросом на утверждение](./media/sharepoint-scenario-summary/09-02-01-allan-email.png)
2. Нажмите кнопку **Approve** (Утвердить). Поток запускает следующий процесс, и пользователь, запрашивающий проект, получает отзыв непосредственно на электронную почту.
