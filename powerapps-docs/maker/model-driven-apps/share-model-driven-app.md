...
## <a name="share-your-app-for-basic-use"></a>Поделитесь своим приложением для базового использования
Чтобы добавить привилегии, чтобы пользователь приложения мог запускать приложение в среде и выполнять общие задачи для своих записей, используйте роль безопасности **Пользователь Common Data Service**.
1.  На сайте [Power Apps](https://web.powerapps.com/?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc) выберите **Приложения**, рядом с приложением на основе модели, которым вы хотите поделиться, выберите **...**, а затем выберите **Поделиться**. 
    > [!IMPORTANT]
    > Шаги для предоставления доступа к приложению на основе моделей отличаются от приложения на основе модели. Инструкции по общему доступу к приложению на основе холста см. в разделе [Поделиться приложением на основе холста в Power Apps](../canvas-apps/share-app.md). 

2.  В **Назначения пользователям роли безопасности** выберите **Безопасность пользователей**.
3.  В списке включенных пользователей выберите пользователей, которым вы хотите предоставить доступ к вашему приложению, а затем на панели команд выберите **Управление ролями**. 
4.  В диалоговом окне **Управление ролями пользователей** выберите роль безопасности **Пользователь Common Data Service**, а затем выберите **ОК**. 

    > [!div class="mx-imgBorder"] 
    > ![](media/share-model-driven-app/select-common-data-service-user.png "Select Common Data Service User")

5. [Добавление ролей безопасности в приложение](#add-security-roles-to-the-app)
6. [Предоставление ссылки на приложение](#share-the-link-to-your-app)

Пользователи с ролью безопасности Пользователь Common Data Service теперь могут получить доступ к вашему приложению. 

## <a name="share-a-model-driven-app-for-specific-use"></a>Совместное использование приложения на основе модели для конкретного использования
В этом разделе вы выполняете задачи для совместного использования приложения на основе моделей с использованием двух ролей безопасности, каждая из которых соответствует потребностям пользователей приложения. Узнайте следующие темы:
...
3. В диалоговом окне **Управление ролями пользователей** выберите ранее созданную роль безопасности **Специалисты по уходу за домашними животными**, затем выберите **ОК**.
...
Необходимо назначить приложению одну или несколько ролей безопасности. Пользователи будут иметь доступ к приложениям на основе назначенных им ролей безопасности.
...
2. В правом нижнем углу плитки приложения выберите **Дополнительно (...)**, затем выберите **Управление ролями**.

    ![Управление ролями для приложения](media/share-model-driven-app/manage-roles.png)

4. В разделе **Роли** можно выбрать, требуется ли предоставить доступ к приложению для всех ролей безопасности или выбранных ролей. Для основного доступа к приложению выберите роль безопасности **Пользователь Common Data Service**. Для более конкретного доступа выберите другой стандарт или пользовательскую или настраиваемую роль безопасности. Например, выберите роли **Планировщики ухода за домашними животными** и **Специалисты по уходу за домашними животными**, которые вы создали ранее в этом разделе. 
...
5. Нажмите кнопку **Сохранить**. 
    ...
    > [!IMPORTANT]
    > Пользователям с привилегиями **Создать** или **Записать** для **Приложение на основе модели** дается доступ ко всем приложениям в среде, даже если они не являются частью какой-либо роли, которая имеет доступ к приложению.
