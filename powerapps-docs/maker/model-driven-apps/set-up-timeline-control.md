...
9.  Выберите модули, которых требуется отображать пользователям. Выберите только те модули, которые требуются вашей организации.
...
10. Укажите следующее в контейнере **Дополнительные параметры**.
    ...
    | По умолчанию разворачивать область фильтра | Установите этот флажок по умолчанию, если хотите отображать панель фильтра в расширенном режиме. <br><br> **Примечание:** когда временная шкала отображается более чем в одном столбце, панель фильтра отображается в виде столбца рядом с записями временной шкалы. Даже если вы сняли флажок **Развернуть панель фильтра по умолчанию** в настройках Временной шкалы, панель фильтра всегда будет отображаться вашим агентам.|
    | Sort | Выберите порядок сортировки, в соответствии с которым записи отображаются на временной шкале. Сортировка основана на поле, которое вы выбрали для действий. Если поле для сообщения, заметок или действия не существует, сортировка выполняется на основе поля **Последнее обновление**. <br><br> Порядок сортировки по умолчанию — **По убыванию**.  <br><br> **Примечание.** Изменение порядка сортировки не изменит свойство времени, отображаемое в элементе управления временной шкалой. Чтобы настроить форму карточки временной шкалы, см. раздел [Настройка формы карточки](#customize-the-card-form).  | По убыванию |
...
В качестве настройщика вы можете выбрать, какие сущности вы хотите показывать пользователям в соответствии с вашими бизнес-требованиями. Для повышения производительности выберите только те действия, которые характерны для бизнеса, и отмените выбор тех действий, которые не используются.
...
В разделе временной шкалы пользователи видят карточку для каждого действия (на основе включенных действий). Каждая карточка отображает определенные поля в свернутом и развернутом режиме. Например, см. карточку действия **Отправить по электронной почте** в свернутом режиме, режиме наведения и в развернутом режиме. 

**Свернутый режим карточки электронной почты**: по умолчанию карточки действия находятся в свернутом режиме.


   > [!div class=mx-imgBorder] 
   > ![Карточка временной шкалы в свернутом режиме](media/email.png "Карточка временной шкалы в свернутом режиме")

**Режим наведения на карточку электронной почты**: при наведении курсора вы видите несколько команд, характерных для каждого типа карточек действий.

   > [!div class=mx-imgBorder] 
   > ![Карточка временной шкалы в свернутом режиме](media/email-hover.png "Карточка временной шкалы в свернутом режиме")

**Развернутый режим карточки временной шкалы**: когда вы выбираете карточку, она раскрывается с помощью нескольких команд, которые являются специфическими для каждого типа карточек действий.
...
Поля **1** и **2** из раздела **Заголовок** и поля **3** и **4** из раздела **Подробности** отображаются в свернутом режиме. Поля **3** и **4** в свернутом режиме не показывают содержимое в формате RTF.
...
**Режим наведения карточки электронной почты**

В режиме наведения отображаются поля **1** и **2** из раздела **Заголовок** и поля **3** и **4** из раздела **Сведения** .

   > [!div class=mx-imgBorder] 
   > ![Карточка электронной почты в свернутом режиме](media/email-card-hover.png "Карточка электронной почты в свернутом режиме")

**Развернутый режим карточки электронной почты**

Поле **5** из раздела **Сведения** отображается в развернутом режиме. В поле **3** в расширенном режиме не отображается содержимое в формате RTF, тогда как поле **4** в расширенном режиме показывает содержимое в формате RTF.
...
5.  Выберите сущность и выберите **Формы**. Например, выберите сущность **Электронная почта**.

6.  Выберите запись **Форма карточки электронной почты** из списка. **Форма карточки электронной почты** открывается в новом окне браузера.
...
## <a name="enable-or-disable-rich-text-editor-for-notes-in-timeline"></a>Включение или отключение редактора форматированного текста для примечаний на временной шкале

Редактор форматированного текста позволяет пользователям создавать богатое и хорошо оформленное содержимое для примечаний с акцентом. Редактор предоставляет такие общие функции текстового процессора. Дополнительные сведения см. в разделе [Создание заметок](https://docs.microsoft.com/dynamics365/customer-service/customer-service-hub-user-guide-basics#take-a-note).

Эта функция включена по умолчанию. Если вы хотите отключить и включить ее позже для своих пользователей, выполните следующие действия:

1.  Войдите в среду `https://<YourOrgURL>.dynamics.com/apps`.

2. Откройте приложение на основе модели, затем на панели команд выберите **Настройки** ![Настройки](../model-driven-apps/media/powerapps-gear.png) > **Администрирование** > **Настройки системы**.

3. В диалоге **Настройки системы** на вкладке **Общие** прокрутите вниз и установите или снимите флажок **Используйте форматированный текст, чтобы облегчить форматирование примечаний, созданных на временной шкале.** .

4. Нажмите **ОК**.

    > [!div class=mx-imgBorder] 
    > ![Включение редактора форматированного текста](media/timeline-note-enable-rich-text-editor.png "Включение редактора форматированного текста")

Редактор форматированного текста включается или отключается для ваших пользователей на основе выбора флажка. 
