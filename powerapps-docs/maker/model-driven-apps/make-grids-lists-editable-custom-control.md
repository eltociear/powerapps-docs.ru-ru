...
Следующие типы данных невозможно отредактировать в редактируемых сетках: поля поиска клиента и Partylist; составные поля (адрес); поля состояния/статус; поля, связанные с сущностью поиска (например, сущность "Организация" включает поиск контакта, где поле "Контакт" редактируемое, но поле EmailAddress(Contact) нередактируемое).  
