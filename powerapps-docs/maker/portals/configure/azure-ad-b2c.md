...
**Имя**: Authentication/UserManager/UserValidator/RequireUniqueEmail

**Описание**: указывается, требуется ли для проверки пользователя уникальный адрес электронной почты. Если для входа используется существующий адрес электронной почты контакта, для этого параметра должно быть установлено значение false. По умолчанию установлено значение *true*, что может привести к неудачным попыткам входа в систему для записей контактов с уже существующим адресом электронной почты.
