...
- Поскольку данные хранятся в Common Data Service, не нужно установить соединение из PowerApps как это происходит с источниками данных, такими как SharePoint, приложения на основе модели в Dynamics 365 или Salesforce. Необходимо только указать сущности которыми требуется управлять или показывать в портал.

- Можно создавать портал с помощью WYSIWYG-приложения Studio порталов PowerApps, добавив и настроив компоненты на веб-страницах.
...
Дополнительные сведения о работе со средами см. в разделе [Работа со средами и Microsoft PowerApps](https://docs.microsoft.com/powerapps/maker/canvas-apps/working-with-environments)
...
Для изменения портала в конструкторе Studio порталов PowerApps см. раздел [Изменение портала](manage-existing-portals.md#edit).
...
> - Порталы PowerApps недоступны в области Франция.
...
3.  Выберите валюту и язык для данных, хранящихся в базе данных. Нельзя менять валюту и язык как только базу данных создается. По завершении нажмите **Создать мою базу данных**. База данных создается с помощью портала начального который позволит вам быстро приступить к работе с пример содержимого после подготовки портала.
    ...
    > [!NOTE]
    > Параметр **Включить начальный портал** доступен только при создании среды с использованием параметра, предоставленного в окне **Портал с нуля**. Этот параметр недоступен при создании среды из центра администрирования PowerApps.
...
4. После создания среды и базы данных в разделе **Создавайте собственные приложения** выберите **Портал с нуля**. 
    ...
    > [!NOTE]
    > Если база данных создана, и вы по-прежнему получаете запрос на создание базы данных, необходимо обновить домашнюю страницу PowerApps, прежде чем выбрать плитку **Портал с нуля**.
