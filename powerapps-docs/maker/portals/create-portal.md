...
5.  В окне **Портал с нуля** введите название портала и адрес веб-сайта, и выберите язык с помощью раскрывающегося списка. По завершении нажмите **Создать**.
    ...
    > [!TIP]
    > Чтобы создать портал на другом языке, вы должны сначала [включить этот язык в среде](https://docs.microsoft.com/power-platform/admin/enable-languages#enable-the-language), чтобы он стал доступен в раскрывающемся списке языков.
...
> [!NOTE]
> - В среде может быть только один портал каждого типа на каждом языке.
