---
title: " Компонент сетки таблицы | Документация Майкрософт"
description: Реализация компонента сетки таблицы
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340214"
---
# <a name="implementing-table-grid-component"></a>Реализация компонента сетки таблицы

В этом образце демонстрируется, как создать простой компонент набора данных, привязку метаданных столбца представления, привязку записи, дополнительные записи от разбиения по страницам и навигации по записям в форму.
Столбцы заголовка компонента и значения внутренних записей привязываются к существующим представлениям.

> [!div class="mx-imgBorder"]
> ![](../media/table-grid-control.png "Компонент") сетки таблицы компонента сетки таблицы
...
Привязка заголовка столбца к представлению:

Просмотр сведений о столбцах зависит от `context.parameters.[dataset_property_name].columns`. Это тип массива.

Привязка записи:

- Идентификаторы отсортированных записей находятся в `context.parameters.[dataset_property_name].sortedRecordIds`
- Все записи находятся в `context.parameters.[dataset_property_name].records` 
- Для каждого объекта записи `context.parameters.[dataset_property_name].records[record_Id]` 
- Отформатированное значение можно получить в `getFormattedValue` 

Загрузить дополнительную страницу данных при необходимости:

`context.parameters.[dataset_property_name].paging` предоставляет функции разбиения на страницы, такие как `hasNextPage` и `loadNextPage` данных. Кнопка `Load More` отображается, если она содержит данные следующей страницы.

В этом примере также демонстрируется, как компонент ожидает изменения размера контейнера. 

Метод `trackContainerResize` должен вызываться в методе [init](../reference/control/init.md) таким образом, чтобы `mode.allocatedWidth` и `mode.allocatedHeight` были предоставлены каждый раз, когда вызывается [упдатевиев](../reference/control/updateview.md) . Если этот метод не вызывается изначально, у него нет `allocatedWidth` и `allocatedHeight` предоставлены.

Если Аллокатедхеигхт имеет значение – 1, то это означает отсутствие ограничения высоты. Компонент должен изменить свою высоту на основе указанной ширины.
