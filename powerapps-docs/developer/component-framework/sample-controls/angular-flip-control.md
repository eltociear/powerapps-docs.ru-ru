---
title: " Перевернуть компонент | Документация Майкрософт"
description: Реализация компонента отражения с помощью угловой JS
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340674"
---
# <a name="implementing-flip-component"></a>Реализация компонента эргономичного перелистывания

В этом примере показано, как использовать сторонние библиотеки для создания компонентов в инфраструктуре компонентов PowerApps.  Компонент выборки с отражением реализуется на основе угловой. js, угловой-UI, угловой-анимация, угловой — очистка, начальная загрузка. Код может не раскрывать рекомендации для упомянутых библиотек сторонних производителей.

> [!div class="mx-imgBorder"]
> ![Угловой]переворот углового(../media/angular-flip.png "вниз")
...
Этот образец содержит примеры добавления зависимостей для сторонних библиотек, демонстрирующих способы выполнения привязки данных между инфраструктурой компонентов PowerApps, моделью компонентов и внутренней моделью данных сторонних поставщиков в двунаправленном режиме.

Пример компонента эргономичного перелистывания состоит из метки и кнопки. При нажатии кнопки текст на метке переключается.

- При загрузке компонента метка отображает текст на основе значения атрибута BIND. @No__t_0 содержит связанные метаданные.
- Для полей Твуптионс `context.parameters.[property_name].Options` будут содержать значения true и false. 
- Нажимая кнопку "перевернуть", метка обновляет значение с помощью метода **нотифйоутпутевентс** , метод " [выводимые выходные данные](../reference/control/getoutputs.md) " будет вызываться асинхронно и будет передаваться в инфраструктуру компонентов PowerApps. 
- Клиентапи обновляет значение атрибута BIND, а обновленное значение передается в метку компонента. Можно также использовать `ClientAPI`, чтобы обновить значение атрибута, чтобы активировать метод [упдатевиев](../reference/control/updateview.md) элемента управления. Затем компонент обновляет модель стороннего производителя, а метка обновляется.
