---
title: " Компонент отправки изображения | Документация Майкрософт"
description: Реализация компонента отправки изображений с помощью typescript
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340720"
---
# <a name="implementing-an-image-upload-component"></a>Реализация компонента отправки изображения

Этот пример компонента отображается как кнопка `Upload`, чтобы передать изображение и изображение по умолчанию, когда компонент загружается в первый раз. Если щелкнуть `Upload`, откроется Проводник, чтобы выбрать образ.
 
Выбранное изображение подготавливается к просмотру в компоненте. В то же время, если необходимо выполнить сброс, отображается кнопка `Remove`. При нажатии кнопки `Remove` отображается изображение по умолчанию.  

> [!div class="mx-imgBorder"]
> ![](../media/image-upload-control.png "Компонент") отправки образа компонента отправки изображения
...
В этом примере показано, как создать средство выбора образа и продемонстрировать API устройства и API ресурсов для загрузки изображения, определенного в манифесте. Содержимое изображения хранится в кодировке Base64 и может быть сохранено и просмотрено.  

Метод `resources.getResource` принимает входные данные в качестве имени ресурса, определенного в манифесте компонента, и загружает этот ресурс. Компонент визуализирует кнопку `Upload` и изображение по умолчанию для начальной отрисовки. Изображения определяются в узле [ресурсов](../reference/resources.md) манифеста.  
...
Будет активировано `successCallback` и содержимое ресурса вставляется в `successCallback`. Затем используется элемент Image "src", указывающий на содержимое и загрузку изображения по умолчанию.
 
Метод `device.pickFile` открывает диалоговое окно для выбора файлов с компьютера (веб-клиента) или мобильного устройства (мобильные клиенты). Для настольного компьютера открывается проводник для мобильного клиента, который открывает библиотеку фотографии. При нажатии кнопки  `upload` API устройства  `pickFile` триггеры и пользователь выбирает файл. После успешного выбора файла имя файла, содержимое файла будет вставлено в `successCallback`. 

> [!NOTE]
> Если в устаревшем веб-клиенте используется одна и та же форма или сущность, то в поле будет отображаться готовый текстовый компонент на устаревшем веб-клиенте, где могут возникнуть проблемы UX.  Чтобы сделать его скрытым на устаревшем веб-клиенте, можно снять флажок **видимость** и установить флажок **скрывать элемент управления по умолчанию** . 
