---
title: " Увеличить компонент | Документация Майкрософт"
description: Реализация компонента инкремента
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340191"
---
# <a name="implementing-increment-component"></a>Реализация компонента инкремента

В этом примере компонента показано, как привязать данные с помощью платформы компонентов PowerApps и обработки ошибок. Этот компонент подготавливается к просмотру как текстовое поле с помощью кнопки `Increment` в среде выполнения. В текстовом поле отображается текущее значение, и кнопка `Increment` нажата. При нажатии кнопки значение в текстовом поле увеличивается на 1. Значение приращения можно изменить на любое нужное число.

Чтобы реализовать этот компонент, сначала необходимо определить файл [манифеста](../manifest-schema-reference/manifest.md) , а затем реализовать пользовательскую логику в TypeScript.

> [!div class="mx-imgBorder"]
> ![Увеличить](../media/increment-control.png "компонент приращения") компонента
...
При нажатии кнопки значение в текстовом поле увеличивается на 1. Обновленное значение будет передаваться в платформу компонентов PowerApps с помощью метода `notifyOutputChanged`.

> [!NOTE]
> Значение приращения можно изменить при настройке компонента на поле в форме.

Измените значение в текстовом поле и, если оно является допустимым целым числом, оно обновляет значение на платформе компонентов PowerApps. Можно последовательно нажать кнопку `Increment` и обновить ее. Если это недействительное целое число, выводится сообщение об ошибке.
