---
title: " Компонент веб-API | Документация Майкрософт"
description: Реализация компонента веб-API
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340168"
---
# <a name="implementing-web-api-component"></a>Реализация компонента веб-API

Компонент веб-API предназначен для выполнения действий по созданию, извлечению, обновлению и удалению. Компонент отображает четыре кнопки, которые можно щелкнуть для вызова различных действий веб-API. Результат вызова веб-API вставляется в элемент HTML div в нижней части компонента Code.  

> [!div class="mx-imgBorder"]
> Компонент веб-(../media/web-api-control.png "API") ![компонента веб-API]
...
По умолчанию в примере компонент настроен для выполнения действий создания, извлечения, обновления для сущности `Account` и установки полей name и доход в примерах веб-API.

Чтобы изменить конфигурацию по умолчанию для любой сущности или поля, обновите следующие значения конфигурации, как показано ниже.  
...
Метод `createRecord` отображает три кнопки, позволяющие создать запись учетной записи с полем "доход", для которой заданы различные значения (100, 200, 300).

При нажатии одной из кнопок «создать» обработчик событий `onClick` кнопки проверяет значение нажатой кнопки и использует действие веб-API для создания записи учетной записи с полем «доход», установленным для значения кнопки. Поле имени записи учетной записи будет иметь значение `Web API code component (Sample)` с произвольным `int`, добавленным к концу строки. Метод обратного вызова из вызова веб-API вставляет результат вызова веб-API (Success или Failure) в результат div пользовательского элемента управления.  
 
Метод `deleteRecord` визуализирует кнопку, при нажатии которой открывается диалоговое окно поиска. В диалоговом окне поиска можно выбрать запись учетной записи, которую нужно удалить. После выбора записи учетной записи из диалогового окна поиска она передается `deleteRecord` для удаления записи из базы данных. Метод обратного вызова из вызова веб-API вставляет результат вызова веб-API (Success или Failure) в результат div пользовательского элемента управления.  

Метод FetchXML `retrieveMultiple` визуализирует кнопку в компоненте кода. `onClick` этой кнопки создается и передается функции `retrieveMultiple`, чтобы вычислить среднее значение поля доход для всех записей учетных записей. Метод обратного вызова из вызова веб-API вставляет результат вызова веб-API (Success или Failure) в результат div пользовательского элемента управления.  

Метод `retrieveMultiple` OData визуализирует кнопку в компоненте кода. `onClick` этой кнопки строка OData создается и передается функции `retrieveMultiple` для получения всех записей учетных записей с полем имени, которое аналогично "веб-API компонентов кода (пример)", что справедливо для всех записей учетной записи, созданных в этом примере компонента кода.  

При успешном извлечении записей компонент кода имеет логику для подсчета количества записей учетной записи со значением поля доход 100, 200 или 300 и отображением этого числа в контейнере состояния OData DIV в компоненте кода.  Метод обратного вызова из вызова веб-API вставляет результат вызова веб-API (Success или Failure) в результат div пользовательского элемента управления.  
