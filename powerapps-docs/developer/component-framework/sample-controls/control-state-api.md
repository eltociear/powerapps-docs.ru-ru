---
title: API состояния элемента управления | Документация Майкрософт
description: ''
keywords: PowerApps Платформа компонентов PowerApps
ms.author: nabuthuk
author: Nkrb
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340536"
---
# <a name="implementing-control-state-api-component"></a>Реализация компонента API состояния элемента управления

Платформа компонентов PowerApps позволяет сохранять состояние компонента в нескольких рендерингах компонента в рамках одного сеанса. Он предоставляет возможность создавать компоненты, которые могут поддерживать пользовательское состояние в сеансе пользователя по мере перехода пользователя к компоненту и его отправителя.

Например, если компонент кода является длинным списком, который пользователь может прокручивать, можно использовать функцию **_сетконтролстате_** для запоминания точки в списке, в которой пользователь смотрит при переходе от формы. Затем можно добавить логику при инициализации компонента для проверки сохраненного состояния и отрисовки списка компонента в точке, в которой пользователь был ранее прочитан.

> [!div class="mx-imgBorder"] 
> (../media/control-state-api.png "API состояния элемента управления") ![API состояния элемента управления]
