---
title: " Компонент таблицы | Документация Майкрософт"
description: Реализация компонента таблицы
ms.custom: ''
manager: kvivek
ms.date: 10/01/2019
...
ms.locfileid: "72340099"
---
# <a name="implementing-table-component"></a>Реализация компонента таблицы

Этот образец компонента отображает таблицу с двумя столбцами. В левом столбце отображается имя метода или свойства API, а в правом столбце отображается значение, возвращаемое API. Этот компонент можно открыть на устройствах различных типов или изменить язык или параметры пользователя, чтобы убедиться, что значения правильно настроены в таблице.

> [!div class="mx-imgBorder"]
> (../media/table-control.png "Компонент") таблицы ![компонентов таблицы]
...
Этот образец содержит примеры использования методов из `IClient, IUserSettings, IUtility, IFormatting interfaces`.

Этот компонент также демонстрирует две служебные функции: `setFullScreen` и `lookupObjects`. Эти функции вызываются нажатием кнопки, отображенной в составе компонента кода. Кнопка `setFullScreen` переключает компонент в полноэкранный режим и из него. Кнопка `lookupObjects` открывает диалоговое окно поиска, а затем вставляет выбранную запись в виде текста в Div.

В этом примере мы отрисовываете кнопку HTML и прикрепите к кнопке обработчик событий `onClick` JavaScript `onLookupObjectsButtonClick`. При нажатии этой кнопки мы вызываем метод `context.utils.lookupObjects()` и передаем в качестве параметра массив имен сущностей. 

Этот метод возвращает объект Promise JavaScript, который представляет завершение или сбой вызова в диалоговом окне поиска. Если обещание разрешено успешно, объект уточняющего запроса, выбранный пользователем, передается в качестве параметра в метод обратного вызова и может быть указан как data.id, data.name, Data. entityType.

Метод обратного вызова вставляет эти сведения как HTML в тег div, отображаемый в компоненте кода, чтобы продемонстрировать выбранные результаты пользователю. Если обещание отклонено, вызывается метод обратного вызова ошибки, где компонент может соответствующим образом обрабатывал сценарий ошибки.
