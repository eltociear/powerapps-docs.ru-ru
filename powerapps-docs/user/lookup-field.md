...
>[!NOTE]   
> Результат поиска по умолчанию для поиска уточняющих запросов —, начинается с. Это означает, что результаты содержат записи, которые начинаются с определенного слова. Например, если вы хотите найти **Alpine Ski House**, введите **Ахметом** в поле поиска. Если ввести **Ski**, запись не отобразится в результатах поиска.
>
> Для поиска с подстановочными знаками используйте звездочки: например, введите * Ski или * Ski.
